...
void ape_fill_buffer(u16* buf,u16 size)
{
	u16 i; 
	u16 *p;
	while(apetransferend==0)delay_ms(2000/OS_TICKS_PER_SEC);//等待传输完成
	apetransferend=0;
	if(apewitchbuf==0)
	{
...
u8 ape_play_song(u8* fname) 
{
	...
	{ 
		...
		{  
			...
			{
				res=2;//压缩率不支持/版本不支持/不是16位音频格式
			}else
			{
				if(apex->compressiontype<=2000)
				{
					OS_ENTER_CRITICAL();//进入临界区(无法被中断打断) 
		      Stm32_Clock_Init(240,8,2,5,4);//设置时钟,144Mhz
          ...
          OS_EXIT_CRITICAL();		//退出临界区(可以被中断打断)	
				...
				{
					 OS_ENTER_CRITICAL();//进入临界区(无法被中断打断) 
	         Stm32_Clock_Init(276,8,2,5,4);//设置时钟,144Mhz
	         delay_init(138);			//延时初始化 
           OS_EXIT_CRITICAL();		//退出临界区(可以被中断打断)
				}
	...
	{   
		...
		res=I2S3_SampleRate_Set(apex->samplerate);		//设置采样率 
		if(res||apectrl->samplerate>48000)//不支持的采样率
			{
        ...
        OS_ENTER_CRITICAL();//进入临界区(无法被中断打断) 
	      Stm32_Clock_Init(192,8,2,4,4);//设置时钟,72Mhz 
	      delay_init(96);			//延时初始化  
        OS_EXIT_CRITICAL();		//退出临界区(可以被中断打断)				
				return AP_ERR;
			}
	...
	myfree(SRAMIN,audiodev.i2sbuf2); 
	 myfree(SRAMIN,buffer);  
	OS_ENTER_CRITICAL();//进入临界区(无法被中断打断) 
	Stm32_Clock_Init(192,8,2,4,4);//设置时钟超频到120Mhz
	delay_init(96);			//延时初始化  
      OS_EXIT_CRITICAL();		//退出临界区(可以被中断打断)	
	return res;
}

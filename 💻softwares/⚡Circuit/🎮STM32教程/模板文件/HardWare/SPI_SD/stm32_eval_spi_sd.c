...
SD_Error SD_GoIdleState(void)
{
  ...
  SD_CS_LOW();
  SD_SendCmd(SD_CMD_SEND_IF_COND,  0x000001CD, 0x55);
  ...
  if (!SD_GetResponse(0x01)){
    ...
    if(buf[2]==0X01&&buf[3]==0XCD) // SD 2.0 card detected 
    {
      ...
      for (uint16_t retry = 0x0FFF; (r2 && retry); retry--){
      ...
      if (r3 == 0x00){
        ...
        SD_ReadBytes(buf2, 4);
        if (buf2[0] & 0x40){
          sd_type = SD_TYPE_V2HC;  // OLED_ShowString(3,1,"SD_HC Card");
        }else{
          sd_type = SD_TYPE_V2;    // OLED_ShowString(3,1,"SD Card");
        }
        return SD_RESPONSE_NO_ERROR;
      }
    }

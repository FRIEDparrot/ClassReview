...
{
    // HAL_SPI_Transmit(&touch_spi_handler, &num, 1, ILI9341_TIMEOUT);
    
    uint8_t count=0;
    for(count=0;count<8;count++)
    {
        if(num&0x80) ILI9341_TDI_SET();
        else ILI9341_TDI_RESET();
        num<<=1;
        ILI9341_TCLK_RESET();
        delay_us(1);
        ILI9341_TCLK_SET();
...
{ 	 
    ...
    TP_Write_Byte(CMD);
    delay_us(6); // ADS7846 maximum convert time is 6us 
...
uint16_t TP_Read_Coor(uint8_t cmd)
{
    ...
    uint8_t  rx_buf[2], tx_buf[2] = {0xff, 0xff};
    uint16_t buf[6];
    uint16_t sum=0;
    ...
    //     delay_us(10);  /* wait for ADS7846 to convert data */
    //     // neglect to give 1 clock for clear BSY bit
    //     HAL_SPI_TransmitReceive(&touch_spi_handler,tx_buf, rx_buf, 2, ILI9341_TIMEOUT);
    //     buf[i] = ((uint16_t)rx_buf[0] << 5) | (rx_buf[1] >> 3);
    //     // only use higher 12 bits , buf[0] is first 7 bytes and buf1 is 8-12 bytes
    // }
    // ILI9341_TCS_SET();

    for( int i=0;i< 6;i++) buf[i]= TP_Read_AD(cmd);	 // read by soft 
...
{   
    ...
    GPIO_InitStructure.Pin  = ILI9341_GPIO_PIN_TIRQ | ILI9341_GPIO_PIN_TDO;
    ...
    GPIO_InitStructure.Pin  = ILI9341_GPIO_PIN_TCLK | ILI9341_GPIO_PIN_TDI;
    GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;           // if use hardware SPI, this line is AF_PP
    GPIO_InitStructure.Alternate = 0; // ILI9341_TOUCH_GPIO_AF;
    HAL_GPIO_Init(ILI9341_TOUCH_GPIO_PORT, &GPIO_InitStructure);
    ...
    // touch_spi_handler.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_128;
    ...
    // if (HAL_SPI_Init(&touch_spi_handler)){
    //     printf("ILI9341_TOUCH_SPI init error!\r\n");
    // }

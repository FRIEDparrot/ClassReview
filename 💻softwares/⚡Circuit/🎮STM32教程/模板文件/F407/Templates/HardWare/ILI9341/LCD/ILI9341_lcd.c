...
#include "stdlib.h"
#define __lcd_malloc malloc
#define __lcd_free free
#else 
#include "../MALLOC/mymalloc.h"
#define __lcd_malloc(x) mymalloc(SRAMIN, x)
#define __lcd_free(p) myfree(p)
#endif

_lcd_dev lcddev;   /* The LCD device structure */

uint16_t POINT_COLOR = 0x0000,BACK_COLOR = 0xFFFF;  
uint16_t DeviceCode;

static SPI_HandleTypeDef lcd_spi_handler;
static uint8_t* lcd_buffer = NULL;    // buffer for the 
...
void __LCD_RD_DATA(uint8_t *data)
{
    ILI9341_CS_CLR();
    ILI9341_DC_SET();
    HAL_SPI_Receive(&lcd_spi_handler, data, 1,  ILI9341_TIMEOUT);
    ILI9341_CS_SET();
}
...
void ILI9341_Clear(uint16_t Color)
{
    ...
    for (uint16_t i = 0; i < LCD_SPI_BUFFER_SIZE; i += 2){
        lcd_buffer[i] = ch;
        lcd_buffer[i + 1] = cl;
    }

    /** send the buffer to the LCD ------------------ */
    for(uint8_t i=0; i < LCD_SPI_BUFFER_DIV; i++)
    {
        ILI9341_CS_CLR();
        ILI9341_DC_SET();
...
{
    if (!lcd_buffer){
        lcd_buffer = __lcd_malloc(LCD_SPI_BUFFER_SIZE);
    }
...
void ILI9341_Fill(uint16_t sx,uint16_t sy,uint16_t ex,uint16_t ey,uint16_t color)
{
    ...
    uint8_t bh = (color >> 8), bl = (color & 0xFF);

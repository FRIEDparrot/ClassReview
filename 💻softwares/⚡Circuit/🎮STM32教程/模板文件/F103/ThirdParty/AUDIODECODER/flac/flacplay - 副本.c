...
u8 flac_init(FIL* fx,__flacctrl* fctrl,FLACContext* fc)
{
	...
	buf=mymalloc(SRAMCCM,512);	//申请512字节内存
...
{ 
	...
	{ 
		...
		if(flacctrl->bps==24)	//24位音频数据
		{
			...
			CS4398_I2S_Cfg(0,1,1,0);//飞利浦标准,24位数据长度
		...
		{
			CS4398_I2S_Cfg(0,1,1,0);//飞利浦标准,16位数据长度
			I2S2_Init(0,2,0,0);//飞利浦标准,主机发送,时钟低电平有效,16位扩展帧长度	
		...
		res=I2S2_SampleRate_Set(fc->samplerate);		//设置采样率 
		if(res)//不支持的采样率
			{
       ...
       */				
			...
			{		  
                ...
                key=KEY_Scan(0,0); 
						...
						if(key==KEY2_PRES||key==KEY1_PRES)//下一曲/上一曲
						{
							res=key;
						  bytesleft=0;//跳出上一级循环
							break; 

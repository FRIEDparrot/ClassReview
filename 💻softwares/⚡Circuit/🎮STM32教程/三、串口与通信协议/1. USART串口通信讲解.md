...
1. 首先初始化时钟和GPIO部分，<b><mark style="background: transparent; color: blue">同时GPIO TX需要配置成复用输出(注意是AF_OD, 不是OUT_OD)-> 因为是在默认复用功能里面</mark></b> 而GPIO RX需要配置为输入(TPU)
2. 配置USART 
...
另外, 中断中写的修改数据可以使用一个变量加Get, 也可以加入全局并且使用extern 进行共享。
...
说明: 由于USART是异步通信, 需要严格控制时序, 所以使用USART时, GDB调试失效。

## 一、栈的链式存储定义方法
### (1) 不同种类栈的定义和使用
对于栈的连续存储的数组定义， 可以参考[[📘ClassNotes/⌨️Programming/🌳C (Data Structure & Algorithms)/C++基础知识/复习 栈和队列|栈的数组定义方法]]
...
因此可以直接通过front来进行访问
[[📘ClassNotes/⌨️Programming/🌳C (Data Structure & Algorithms)/C++基础知识/复习 栈和队列#一、队列|连续存储队列的定义]]

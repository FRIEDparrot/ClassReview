...
需要说明的是, 对于稀疏矩阵的构建, 如果采用一个稀疏矩阵提取包含训练集和测试集的矩阵, 可能会显得比较麻烦, 此处参考[[📘ClassNotes/⌨️Programming/🐍Python/🌟数据集训练补充知识/1. Scipy 稀疏矩阵相关内容|1. Scipy 稀疏矩阵相关内容]] 进行构建稀疏矩阵; 
...
参考[[📘ClassNotes/⌨️Programming/🐍Python/🌟数据集训练补充知识/2. 近似最邻近算法(kdtree,Faiss,Annoy, ScaNN)|2. 近似最邻近算法(kdtree,Faiss,Annoy, ScaNN)]], 采用如下计算 Transform 之后的每一项的距离, 并聚类到对应的 User 组中.
```python

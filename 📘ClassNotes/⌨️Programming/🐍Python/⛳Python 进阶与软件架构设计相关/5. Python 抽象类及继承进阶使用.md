...
## 二、Python 抽象类的使用方法
在 Python 中, 抽象类是一种无法被实例化的类，用于定义通用的接口或行为，具体实现由子类完成。抽象类的核心功能是通过定义抽象方法，确保子类必须实现这些方法，从而实现一种协议或规范。例如, 在 VB 中, 通过 MustInherit 和 MustOverride  实现抽象类型, 参考[[💻softwares/🖥️Programming/📺 VisualBasic/结构体模拟 Union 类型#MustInherit 和 MustOverride|MustInherit 和 MustOverride]] 

...
#### 3. 交流铁芯线圈电路
对于电阻为R的线圈, 通入的电压$u$, 电流$i$, 产生主磁通$\Phi$和漏磁通$\Phi_{\sigma}$, 设主磁通$\Phi = \Phi_{m} \sin \omega t$, 有关系: 
...
显然$X$随着$s$的增大而增大, 即最大为$2\pi f_{1}L_{2}$， 因此容易计算出感应电流有效值和功率因数:
$$\begin{cases}
...
功率因数$\cos \varphi_N$
额定功率$P_{2N}$: 电动机轴上的输出功率，有公式:
$$\eta = \frac{P_{2N}}{P_{1N}} \cdot 100\% \qquad  P_{1N} = \sqrt{3} U_{1N}I_{1N}$$

### (5) 三相异步电动机的启动
对于小型的轻载电机, 可以直接启动,  但是更多是**降压启动** 


主要有Y-$\Delta$ 降压启动(适用于轻载启动, 不会产生较大电流)和串电阻启动(启动之后 切除外电阻, 适用于重载启动情况)

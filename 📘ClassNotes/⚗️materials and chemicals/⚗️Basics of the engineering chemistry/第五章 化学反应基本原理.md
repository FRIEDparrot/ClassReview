...
例如求解298K下的$Ag_{2}O$能否分解为$Ag, O_{2}$时, 由于$O_{2}$分压不同，需要按照分压代入。
...
## 二、化学平衡与移动 
### (1) 标准平衡常数及其计算
在化学反应在进行时，由于反应物的浓度减小和产物的浓度增大， $\Delta G$由**负值逐渐向0靠拢**, 当达到$\Delta G= 0$时, 各个反应物和产物的浓度不再变化， 此时<mark style="background: transparent; color: red">相对应的反应熵是一个定值, 称为标准平衡常数</mark>$K^{\theta}$, 其中使用平衡时的浓度和平衡时的 
...
**若升温, 平衡向吸热方向移动**， 而若**降温， 则平衡向放热方向移动**

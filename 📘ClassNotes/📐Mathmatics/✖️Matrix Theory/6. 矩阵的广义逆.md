...
$$ \Large \boxed{F^{+} = (F^{H}  F)^{-1} F^{H}, \qquad  F^{+} F = I_{r}} \tag{1.1.5} $$
$$ \Large \boxed{G^{+}  = G^{H}(GG^{H})^{-1 }\qquad  GG^{+}= I_{r}} \tag{1.1.6} $$
...
\Sigma _{r} & O   \\  O &O
\end{matrix}\right) V^{H} \tag{1.1.8}
$$
则有关系:
$$
A^{+} = V \left[\begin{matrix}
\Sigma_{r}^{-1} &  O \\  O & O
\end{matrix}\right] U^{H} \tag{1.1.9}
$$
...
I_{r}  &  O  \\  O & L
\end{matrix}\right]Q \tag{1.1.11}
$$
是 A 的 $\{1 \}-$ 逆, 而特别地, 有 $n \times m$ 矩阵
$$
X = P\left[\begin{matrix}
I_{r} & O  \\  O & O
\end{matrix}\right] Q \tag{1.1.12}
$$
是 $A$ 的 $\{1, 2\}-$ 逆。
...
$$ R(AB) \subset  R(A) , N (B) \subset  N (AB) \tag{1.2.1} $$
...
1. 
$$ (A^{(1)})^{H}  \in  A^{H}\{1\} \tag{1.2.3} $$
2. 
$$ \lambda^{+} A^{(1)} \in  (\lambda A)\{ 1\} \tag{1.2.4} $$
3. 若 $S$ 和 $T$ 均可逆, 则有 $T^{-1} A^{(1)} S^{-1} \in  (SAT)\{ 1\}$ 成立 
4. 秩相等关系:
$$ \text{rank} A^{(1)}  \geq \text{rank} A \tag{1.2.5} $$
5. $A A^{(1)}$  和 $A^{(1)} A$ 均为幂等阵, 且与 $A$ 秩相同。
$$ R(AA^{(1)}) = R(A),  N(A^{(1)} A)  = N(A) \tag{1.2.6} $$
6.  $AA^{(1)}$ 和 $A$ 的空间相等关系:
$$ R(AA^{(1)}) =  R(A), N(A A^{(1)}) = N(A) \tag{1.2.7} $$
...
$$ \text{rank} A^{H} A  =  \text{rank} A  = \text{rank} AA^{H} \tag{1.2.8} $$
<b><mark style="background: transparent; color: orange">定理 6.8</mark></b> 给定矩阵 A, 则有:
$$ Y = (A^{H} A )^{(1)} A^{H} \in  A\{ 1,2,3\} \tag{1.2.9} $$
$$ Z = A^{H}(AA^{H})^{(1)} \in A\{ 1,2,4\} \tag{1.2.10} $$
...
$$ \Large \boxed{(A \oplus B)^{+} = A^{+} \oplus B^{+}} \tag{1.2.11} $$
...
$$ A^{+} =  (A^{H} A)^{-1} A^{H} \tag{1.2.12} $$
**如果有 $A\in C_m^{m \times  n}$ (行满秩), 则**有:
$$ A^{+} = A^{H} (AA^{H})^{-1} \tag{1.2.13} $$

<b><mark style="background: transparent; color: orange">推论2(推论1特例)</mark></b> :  设 $\alpha$ 为 n 维非零列向量(列满秩), 且有 $\alpha \neq 0$, 则有:
$$ \alpha^{+}  = (\alpha^{H} \alpha )^{-1} \alpha^{H} \tag{1.2.14} $$
$$ (\alpha^{H})^{+}  = (\alpha^{+})^{H} = \alpha (\alpha^{H} \alpha)^{-1} \tag{1.2.15} $$
### (3) 投影和广义逆的求法
...
$$ \boxed{G^{(i)} F^{(1)} \in A\{ i\},  i = 1,2,4} \tag{2.1.1} $$
$$ \boxed{G^{(1)} F^{(i)} \in  A\{ i\} ,  i = 1,2,3} \tag{2.1.2} $$
$$ \boxed{G^{(1)} F^{+}\in A\{ 1,2,3\},  G^{+} F^{(1)} \in  A\{ 1,2,4\}} \tag{2.1.3} $$
$$ \boxed{A^{+} =  G^{+} F^{(1,3)}  =G^{(1,4)} F^{+} } \tag{2.1.4} $$
$$ \boxed{A^{+} = G^{+ } F^{+} = G^{H} (GG^{H})^{-1} (F^{H} F)^{-1} F^{H} } \tag{2.1.5} $$
广义逆矩阵与线性方程组的求解


<b><mark style="background: transparent; color: orange">引理6.5</mark></b>: 设 $A \in  C_{r}^{m\times n} ,  U \in C^{n \times  p},   V\in C^{q\times  m}$, 且有
$$ X = U (VAU)^{(1)} V \tag{2.1.6} $$
则有:
...
$$ A\{ 1,4 \} = \{A^{(1,4)} + Z (I - AA^{(1,4)}) | Z \in  C^{n \times  m} \} \tag{3.3.2} $$
为 $A\{1,4\}$ 部分的通解。
...
$$ \boxed{\LARGE A^{H} A x =   A^{H} b} \tag{3.4.4} $$
> [!NOTE] 说明
...
$$ AA^{(1)} DB^{(1)}B = D \tag{3.7.1} $$
对于$Ax = b$首先, 通过如下方法检查是否有解(相容):
$$ A A^{(1)}  b = b   \qquad \leftrightarrow  \qquad  Ax = b\space 相容 \tag{3.7.2} $$
有解时, 其通解为
$$ x =  A^{(1)} b + (I - A^{(1)} A )y \tag{3.7.3} $$
> [!HINT] 说明

...
#### 2. 子空间的直和 
<b><mark style="background: transparent; color: blue">推论1</mark></b>: 如果 $V_1 + V_2$ 中的任意向量**只能唯一表示为 $V_1$ 的一个向量和$V_2$中的一个向量的和**, 则称对应的元素 $V_1 + V_2$ 为$V_1$ 和 $V_2$ 的直和,并记为$V_{1}\oplus V_{2}$，即: 
$$x = x_{1} + x_{2} = y_{1}  + y_{2}$$
<b><mark style="background: transparent; color: blue">推论2</mark></b>: 如果 $x_1, x_2, \dots  x_k$ 为 $V_1$ 的基， y_1, y_2 , y_l 为V_2的基, 且 $V_1 + V_2$ 为直和, 则有 $x_1, x_2,\dots x_k, y_1,  y_2 , \dots  y_l$ 为 $V_1 \oplus  V_2$的基。
...
> [!NOTE] 特殊的线性变换
> 1. 单位变换(恒等变换) $T_e$ 有 $T_e(x) = x, \forall x \in  R$

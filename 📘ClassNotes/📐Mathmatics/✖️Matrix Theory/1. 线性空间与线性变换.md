...
$$ \sigma(a) = a'\qquad \text{or} \qquad  a \rightarrow  a' \tag{1.1.1} $$
a为原像(pre-image), a'为像.

常见的映射包含单射, 满射, 双射(一一对应)等, 参考[[📘ClassNotes/📐Mathmatics/📈Advanced Mathematics/第一章 集合,数列,函数,极限和连续#(3) 映射|集合,数列,函数,极限和连续]] 

例如: 对于数域 K 有:
$$ \sigma_{1}(A) =  \det A(满射) , A\in  K^{n \times  n}\qquad  \sigma_{2}(a) = aI , a\in  K (单射)\qquad \sigma_{3}(f(t))= f'(t),f(t)\in P_{n} \text{(非满射,  非单射)} \tag{1.1.2} $$
> [!NOTE] $P_n$ 说明
...
$$ (\tau\sigma) (a) = \tau(\sigma (a))\qquad  a \in  S \tag{1.1.3} $$
...
$$ \boldsymbol{x} = c_{1} \boldsymbol{x}_{1} + c_{2}\boldsymbol{x}_{2}+ \dots  + c_{m} \boldsymbol{x}_{m} \tag{1.3.1} $$
...
$$ \boxed{ \Large c_{1}x_{1} + c_{2}x_{2} + \dots  + c_{m}x_{m} = 0 } \tag{1.3.2} $$
则称向量组 $x_1, x_2, \dots  x_m$ 线性相关, 否则称向量组 $x_1, x_2, \dots  x_m$ 为**线性无关**。 

性质
1. 一个元素线性相关的充要条件是这个元素是0元
数域的维数为构成**某个数域的线性无关的向量个数**, 例如:
$$ \text{dim} C^{n} = n, \text{dim} R^{n} = n, \text{dim} K^{m\times  n} = mn, \text{dim} P_{n}= n +1 \tag{1.3.3} $$
...
$$ (\boldsymbol{y}_{1}, \boldsymbol{y}_{2}, \dots  \boldsymbol{y}_{m}) = (x_{1}, x_{2}, \dots x_{n})B , \qquad  B = (b_{1,}b_{2}, \dots  b_{m}) \tag{1.3.7} $$
...
$$
则有基变换关系:
$$ \beta^{T} = C^{T} \alpha^{T}  \rightarrow  \boldsymbol{\beta} =   C \alpha \rightarrow  (\beta_{1}, \beta_{2}, \dots  \beta_{n}) =  (\alpha_{1}, \alpha_{2}, \dots \alpha_{n} ) C \tag{1.3.9} $$
即有:
$$ \boxed{\Large \beta  = \alpha C} \tag{1.3.10} $$
...
$$ (x_{1}, x_{2}, \dots  x_{n})^{T}  = C(y_{1}, y_{2}, \dots y_{n} )^{T} \tag{1.3.11} $$
同时有 : 矩阵 $C$ **可逆**。写成矩阵形式即有: 
$$ \boxed{\Large x = Cy\qquad  \text{ or }  \qquad y =C^{-1}x} \tag{1.3.12} $$
...
$$ \Large \boxed{V_{1} =\left\{ k_{1} \alpha_{1}  + \dots  + k_{m}  \alpha_{m}  | k_{i} \in  K, i =1,2 \dots m\right\}} \tag{2.1.1} $$
是 $V$ 的**线性子空间**, 记为 向量组 $\{\alpha_1, \alpha_{2}, \dots  \alpha_m \}$ 生成的子空间。并记为:
$$ \text{span}  \left\{ \alpha_{1}, \alpha_{2}, \dots  \alpha_{n}\right\}\qquad  \text{or} \qquad  L(\alpha_{1}, \alpha_{2}, \dots \alpha_{n}) \tag{2.1.2} $$
> [!hint] 说明
...
$$ \text{span}\left\{ \alpha_{1}, \alpha_{2}, \dots  \alpha_{s}\right\} \subset  span  \left\{\beta_{1}, \beta_{2} , \dots  \beta_{t} \right\} \tag{2.1.3} $$
若$\alpha_1, \alpha_2, \dots  \alpha_s$ 和 $\beta_1, \beta_2, \dots \beta_s$ 等价, 则:
$$ \text{span} \left\{ \alpha_{1}, \alpha_{2}, \dots \alpha_{s}\right\} =\text{span} \left\{\beta_{1}, \beta_{2}, \dots \beta_{t} \right\} \tag{2.1.4} $$
...
$$ R(A) = \left\{ Ax|x \in K^{n} \right\}, \quad   R(A^{T}) =  \left\{A^{T} y| y \in K^{m} \right\} \tag{2.1.5} $$
...
$$ n(A) = \dim N(A) = n - \text{rank} A\qquad  n 为  A 的列数 \tag{2.1.6} $$
$$ n(A)- n(A^{T}) = A列数  - A 行数 \tag{2.1.7} $$
...
$$ w = k_{1} y_{1} + k_{2} y_{2} + l_{1}x_{1} + l_{2}x_{2} \tag{2.2.1} $$
此时可以找出
$$ x = k_{1}y_{1} + k_{2}y_{2} = l_{1}x_{1}+ l_{2}x_{2} \tag{2.2.2} $$
解此方程, 即**解出最终的 $k_1, k_2, l_1, l_2$ 的约束条件**, 

<b><mark style="background: transparent; color: blue">维数公式</mark></b>: 如果 $V_1, V_2$ 是 $V$ 的两个子空间, 则有关系:
$$ \Large \boxed{\dim V_{1} + \dim V_{2} = \dim (V_{1} + V_{2}) + \dim (V_{1} \cap  V_{2})} \tag{2.2.3} $$
...
$$ x = x_{1} + x_{2} = y_{1}  + y_{2} \tag{2.2.4} $$
...
$$ T(\alpha + \beta) = T(\alpha) + T(\beta) \qquad  T(k\boldsymbol{\alpha}) = k T(\boldsymbol{\alpha}) \tag{3.1.1} $$
则称 $T$ 是 $V$ 的一个**线性变换**.
...
$$ \boxed{R(T) = span\left\{ T(\alpha_1), T(\alpha_2), \dots T(\alpha_n) \right\}} \tag{3.1.3} $$
2. 线性变换的秩等于其<mark style="background: transparent; color: red">象空间的秩和核空间的秩之和总是等于全空间的维数</mark>n ($rankT + nullT = n$)
$$ \dim  R(T)+ \dim N (T) = n \tag{3.1.4} $$

### (2) 线性变换的运算
设 $T_1, T_2$ 是V的两个线性变换, 定义和 $T_{1} + T_2$ <mark style="background: transparent; color: red">加法和数乘运算</mark>为:
$$ (T_{1}+ T_{2}) (x) = T_{1}(x) + T_{2}(x) \tag{3.2.1} $$
$$ (kT_{1})(x) = k(T_{1}(x)) \tag{3.2.2} $$
定义两个线性变换 $T_1, T_2$ 乘积为:
$$ (T_{1} T_{2}) (\alpha) = T_{1} (T_{2}(\alpha))\qquad  \alpha \in V \tag{3.2.3} $$
...
$$ U(S +T)  = US+UT\qquad  (S+T)U = SU+ TU \tag{3.2.4} $$
...
#### 2. 多项式定义
设数域 $K$ 中有多项式:
$$ \boxed{f(T) = a_{m}T^{m} + \dots  + a_{1} T +a_{0} I} \tag{3.3.4} $$
而 T 是线性变换, 则称其为**线性变换 $T$ 的 m 次多项式**

性质：对于 $h(t) = f(t) + g(t), p(t) = f(t) + g(t)$, 则**同一**线性变换的多项式相乘是顺序是可以交换的，有:
$$ h(T) =  f(T) g(T) = g(T) d (T)\qquad p(T) =  f(T) +g(T) \tag{3.3.5} $$
成立

## 四、线性变换的矩阵表示
### (1) 矩阵表示及其性质
定义: 如果将线性变换 $T$ 表示为矩阵 $A$, 则:
$$ \boxed {\Large T(x_{1}, x_{2}, \dots  x_{n}) = (Tx_{1}, Tx_{2} \dots Tx_{n}) = \{x_{1}, x_{2}, \dots  x_{n}\} A } \tag{4.1.1} $$
...
$$ \Large\boxed{\dim R (T) = \dim R (A), \qquad  \dim N(T) = \dim N(A)} \tag{4.1.2} $$
...
$$ (T_{1} + T_{2}) (x_{1}, x_{2}, \dots  x_{n}) = (x_{1}, x_{2}, \dots  x_{n}) (A + B) \tag{4.1.3} $$
$$ kT_{1} (x_{1}, x_{2}, \dots  x_{n}) = (x_{1}, x_{2}, \dots  x_{n}) (k A) \tag{4.1.4} $$
$$ T_{1} T_{2}(x_{1}, x_{2}, \dots  x_{n}) = (x_{1}, x_{2}, \dots  x_{n}) (AB) \tag{4.1.5} $$
$$ T_{1}^{-1}(x_{1}, x_{2}, \dots  x_{n}) =  (x_{1}, x_{2}, \dots  x_{n}) A^{-1} \tag{4.1.6} $$
...
$$ T(x_{1}, x_{2} ,   \dots  x_{n}) = (x_{1}, x_{2,}\dots  x_{n}) A \tag{4.1.7} $$
则有 $V^n$ 的线性变换 $f(T)$ 在基下的矩阵为:
$$ f(A) =  a_{0} A^{m}  + a_{1}  A^{m-1} + \dots   a_{m-1} A +   a_{m} I \tag{4.1.8} $$
并称为**方阵 $A$ 的多项式**. 
...
$$ \Large\boxed{(\eta_{1}, \eta_{2},\dots \eta_{n})^{T} = A (\xi_{1,}\xi_{2}\dots   \xi_{n})^{T}} \tag{4.1.9} $$
即有==变换前后的坐标关系== 
$$ \Large\boxed{\beta =    A \alpha } \tag{4.1.10} $$
...
$$ (y_{1},  \dots y_{n}) = (x_{1}, \dots x_{n}) C \tag{4.1.11} $$
则有下式成立(即**A与B相似且矩阵为C**):  
$$ \boxed{\Large B = C^{-1} A C} \tag{4.1.12} $$
...
$$ V_{\lambda_{0}} = \left\{ x | Tx   = \lambda_{0} x , x\in  V^{n}\right\} \tag{4.3.1} $$
其中是子空间的性质是容易证明的:
$$ T (x + y)  =  T x + T y  = \lambda_{0} (x + y) \qquad  (x + y) \in  V \tag{4.3.2} $$
$$ T(kx) = k(Tx) =  k (\lambda_{0} x), \qquad  k(Tx) \in V \tag{4.3.3} $$
特征子空间的性质: 
...
$$ x_{11}\dots  x_{1r_{1}} , x_{m1}\dots  x_{m r_{m}  } \tag{4.3.4} $$
也**线性无关**
4. <mark style="background: transparent; color: red">矩阵A 的所有特征值之和等于 A 的迹, 特征值之积等于 det A</mark> : 对于特征多项式 $\varphi(\lambda)$, 可以利用行列式展开, 有:
$$ \varphi(\lambda) =  \det (\lambda I - A) = \lambda^{n} - (a_{11} + a_{22} +  \dots  + a_{nn}) \lambda^{n-1}  + \dots  = 0 \tag{4.3.5} $$
...
$$ \boxed{\sum_{i = 1}^{n} \lambda_{i} = \sum_{i = 1}^{n} a_{ii} = \text{tr} A \qquad   \lambda_{1} \lambda_{2}\dots  \lambda_{n}   = \det  A} \tag{4.3.6} $$
...
$$ \Large\boxed{\lambda^{n} \varphi_{AB}(\lambda) = \lambda^{m} \varphi_{BA}(\lambda)} \tag{4.4.1} $$
...
$$ \varphi (\lambda ) = \det(\lambda I-A)  = \lambda^{n} + a_{n-1} \lambda^{n-1}+  a_{1} \lambda + \dots  + a_{0} \tag{4.4.3} $$
时, 则有:
$$ \Large \boxed{\varphi (A) =  A^{n} + a_{n-1} A^{n-1} +  \dots +  a_{1} A +a_{0} I = O} \tag{4.4.4} $$
另外,**根据上述定理, 容易得到**<mark style="background: transparent; color: red">逆矩阵的矩阵多项式表示方法</mark>:
$$ {A^{-1} =  - \frac{1}{a_{0} }(A^{n-1 } + a_{n-1} A^{n-2} +  \dots  + a_{2} A +  a_{1})} \tag{4.4.5} $$
...
$$ m(\lambda) = \frac{\varphi(\lambda)}{d(\lambda)} \tag{4.4.6} $$
...
$$ V^{n} =  V_{1}\oplus   V_{2} \oplus  V_{3}  \dots \oplus V_{s} \tag{5.2.1} $$
又: 我们在每个不变子空间 $V_i$ 中取基: 
$$ x_{i1}, x_{i2} , x_{i3} \dots  x_{in_{i}}  \qquad (i = 1, 2, \dots  s) \tag{5.2.2} $$
则将其合并作为 $V^n$ 的基, 则 $T$ 在该基下的矩阵为: 
$$ A = \text{diag} (A_{1}, A_{2}, \dots  A_{s}) \tag{5.2.3} $$
...
$$ \varphi(\lambda) = (\lambda - \lambda_{1})^{m_{1} }(\lambda - \lambda_{2})^{m_{2}} \dots  (\lambda - \lambda_{s})^{m_{s}} \tag{5.3.1} $$
$$ (m_{1}+m_{2}  + \dots + m_{s} = n) \tag{5.3.2} $$
则有 $V^n$ 可以分解为不变子空间的直和
$$ V^{n} = V_{1}\oplus V_{2}\oplus V_{3} \dots  \oplus V_{s} \tag{5.3.3} $$
...
\end{matrix}\right]_{m_{i} \times  m_{i}}\qquad (i = 1,2, \dots  s) \tag{5.3.5}
$$
在**不考虑顺序时, 若当标准型是唯一的** 
...
-j  & 1  \\ &  -j  & 1  \\ && -j
\end{matrix}\right]
\end{matrix}\right] \tag{5.3.6}
$$

<b><mark style="background: transparent; color: blue">定理</mark></b>: **设 $A$ 为 $n$ 阶复矩阵，且其特征多项式的某种分解式为**
$$ \varphi(\lambda) = (\lambda - \lambda_{1})^{m_{1}}(\lambda - \lambda_{2})^{m_{2}}\dots (\lambda - \lambda_{s})^{m_{s}} \qquad(m_{1}+m_{2}  + \dots + m_{s} = n)\quad  (\star) \tag{5.3.7} $$
**则存在 $n$ 阶可逆复矩阵 $P$, 使得**:
$$ P^{-1} A P =  J \tag{5.3.8} $$
...
\end{matrix}\right] \tag{5.4.3}
$$
同时, **每一项的多项式**有<b><mark style="background: transparent; color: blue">整除关系</mark></b>:
$$ \boxed {d_{1}(\lambda) | d_{2}(\lambda) , \quad  d_{2}(\lambda) | d_{3} (\lambda) , \dots  d_{s-1}(\lambda)  |  d_{s}(\lambda)\qquad  (s \leq n)} \tag{5.4.4} $$
...
$$ d_{i}(\lambda) = \frac{D_{i}(\lambda)}{D_{i-1} (\lambda)} \qquad  D_{0}(\lambda) = 1( i = 1,2,  \dots s) \tag{5.4.5} $$
...
$$ A(x_{1} , x_{2},\dots  x_{n}) = (x_{1} , x_{2},\dots  x_{n}) J \tag{5.5.2} $$
...
$$ (x,y) = \xi_{1}\eta_{1} + \xi_{2}\eta_{2}  +  \dots  + \xi_{n} \eta_{n} = x y^{T} \tag{6.1.1} $$
此时可以验证 $(x,y)$ 满足内积的条件,  
对于实线性空间 $C(a,b)$, $f(t)$ 与 $g(t)$ , 我们规定:
$$ (f(t), g(t)) = \int_{a}^{b} f(t) g(t)dt \tag{6.1.2} $$
...
$$ (A, B) = \sum_{i,j = 1}^{n} a_{ij}b_{ij} = \text{tr} (AB^T) \tag{6.1.3} $$
也满足条件, 因而也是欧式空间
...
$$ (\boldsymbol{x},\boldsymbol{y})  = \left(\sum_{i = 1}^{n} \xi_{i}x_{i} ,  \sum_{j = 1}^{n} \eta_{j} x_{j}\right)= \sum_{i,j = 1}^{n} \xi_{i}\eta_{j} (x_{i}, x_{j}) \tag{6.1.4} $$
上式的非负性只需取 
$$ (x,x) = \left|\left|\sum_{i = 1}^{n}\xi_{i}x_{i}\right|\right|^{2} >  0 \tag{6.1.5} $$
...
$$ |x| = \sqrt{\xi_{1}^{2} +  \dots + \xi_{n} ^{2}} \tag{6.1.8} $$
而<b><mark style="background: transparent; color: orange">对于欧式空间</mark></b> $C(a,b)$ 而言, 向量 f(t) **长度由下式给出**: 
$$ \left| f(t)\right| = \sqrt{\int_{a}^{b} f^{2}(t) dt} \tag{6.1.9} $$
其中长度为 1 的向量为单位向量. **单位化**(规范化)只需 $x_{0} = \frac{x}{|x|}$,同理定义**夹角和余弦**:
$$ \cos <x, y> = \frac{(x,y)}{|x| |y| } \qquad <x,y> = \arccos \frac{(x,y)}{|x| |y|} \tag{6.1.10} $$
容易证明: 
$$ \left| (x,y) \right| \leq  |x| |y| \tag{6.1.11} $$
上述不等式推广到欧式空间 $C(a,b)$, 则成为:
$$ \left| \int_{a}^{b} f(t) g(t) dt \right| \leq  \left[ \int_{a}^{b} f^{2}(t) dt\right]^{\frac{1}{2}}   \left[\int_{a}^{b} g^{2}(t) dt\right]^{\frac{1}{2}} \tag{6.1.12} $$
...
$$ 1, \cos t,  \sin t, \cos 2t , \sin 2t \dots \tag{6.2.1} $$
显然有
$$ \int_{-\pi}^{\pi}  f(t) g(t) dt = 0 \tag{6.2.2} $$
...
$$ \left|x + y \right|^{2}  = \left| x\right|^{2} + |y|^{2} \tag{6.2.3} $$
显然容易推广到多个向量
...
$$ \Large \boxed{(Tx , Ty )  =  (x,y)} \tag{6.2.7} $$
成立
...
$$ (y_1, y_2,\dots y_{n} ) =  (x_{1}, x_{2}, \dots  x_{n}) A \tag{6.2.9} $$
则 $A$ 是正交矩阵。
...
$$ V^{n} = V_{1 }  \oplus V_{1}^{\perp} \tag{6.2.10} $$
...
$$
我们引入向量:
$$ x = (\xi_{1}, \xi_{2}, \dots \xi_{n} ) , x_{i} = (a_{i1}, a_{i2}  , \dots  a_{in}) \tag{6.2.12} $$
则上述可改写为:
$$ (x_{1}, x) = 0,  (x_{2,}x )  = 0 , \dots  (x_{n}, x ) =0 \tag{6.2.13} $$
此时 $x$ 即为上述方程组的解向量, 并计$x_1,\dots x_m$ 生成子空间为:
$$ \text{span} (x_{1}, x_{2}, \dots   x_{m}) \tag{6.2.14} $$
...
$$ R^{\perp}(A) = N(A^{T}),\quad  R(A) \oplus  N(A^{T}) = R^{m} \tag{6.2.15} $$
$$ R^{\perp} (A^{T}) = N(A), \quad  R(A^{T}) \oplus N(A) = R^{n} \tag{6.2.16} $$
在上述定理中, 当 $A \in C^{m \times  n}$ 时, 则有
$$ R^{\perp}(A) = N(A^{H}),\quad  R(A) \oplus  N(A^{T}) = C^{m} \tag{6.2.17} $$
$$ R^{\perp} (A^{H}) = N(A), \quad  R(A^{T}) \oplus N(A) = C^{n} \tag{6.2.18} $$
仍然成立。(其中$A^H$为 A 的**共轭转置**)

#### 4. 对称变换
定义: 设 T 是欧式空间 $V$  的一个线性变换, 且有对于 V 中任意两个向量 $x,y$, 有
$$ (Tx, y ) = (x, T y) \tag{6.2.19} $$
则称 $T$ 为 $V$ 中的一个对称变换
...
$$ (x,y) = \xi_{1} \overline{\eta_{1}} +\xi_{2} \overline{\eta_{2}} + \dots  + \xi_{n}\overline{ \eta_{n}} \tag{6.3.1} $$

仍然满足类似的<b><mark style="background: transparent; color: orange">内积的基本性质</mark></b>:
1. $(x,ky) = k(x,y)$
2. $(x,0) = (0,x) = 0$ 
3. 取两个基为 $x_i, y_i$, 并将空间$V^n$ 中任意两个向量 $x,y$ 表示为:
$$ (\boldsymbol{x},\boldsymbol{y})  = \left(\sum_{i = 1}^{n} \xi_{i}x_{i} ,  \sum_{j = 1}^{n} \eta_{j} x_{j}\right)= \sum_{i,j = 1}^{n} \xi_{i}\overline{\eta_{j}} (x_{i}, x_{j}) \tag{6.3.2} $$
4. 称 $\sqrt{(x,x)}$ 称为向量 $x$ 的<mark style="background: transparent; color: red">长度或范数</mark>, 记为 $|x|$ 或 $||x||_2$, 
5. 对于任意两个向量仍然有
$$ (x,y) (y,x) \leq (x,x) (y,y) \tag{6.3.3} $$
6. 复空间中, 定义夹角的概念为:
$$ \boxed{\cos^{2} <x,y>  = \frac{(x,y)(y,x)}{(x,x) (y,y)}} \tag{6.3.4} $$
...
$$ (Tx  , Tx) = (x,x) \quad  (x \in  V) \tag{6.4.1} $$
则称 $T$ 为 $V$ 的**酉变换**, 并有:
...
$$ (Tx, y) = (x,Ty)\quad  (x,y\in V) \tag{6.4.6} $$
...
$$ P^{H} A P = \Lambda  = \text{diag} (\lambda_{1} , \lambda_{2},  \dots  \lambda_{n} ) \tag{6.4.8} $$
此时，类似特征向量地, 划分 $P$ 为:
$$ P = (p_{1},  p_{2}, \dots p_{n}) \tag{6.4.9} $$
此时有关系:
$$
...
\end{matrix}\right] = \lambda_{1}p_{1}p_{1}^{H} + \dots  + \lambda_{2}p_{2}p_{2}^{H} \tag{6.4.10}
$$
此时称上式为 **Hermite 矩阵 $A$ 的谱分解**，而取 
$$ B_{i} = p_{i} p_{i}^{H} \tag{6.4.11} $$

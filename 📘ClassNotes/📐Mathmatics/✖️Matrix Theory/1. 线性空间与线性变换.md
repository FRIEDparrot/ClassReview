...
设 $T$ 是 $V$ 的线性变换, 集合 $R(T) = \left\{Tx| x\in V \right\}$ 称为线性变换 $T$ 的**值域**, 即有:$R(T) = \left\{ Tx|x\in V \right\}$ 集合 $\left\{x |Tx = 0, x\in V \right\}$ 称为 T 的**核**, 并采用 $N(T)$ 进行表示。其中也称 $R(T)$ 为 $T$ 的**象空间**， $N(T)$ 也称为核空间或者**零空间** 

**定理** : 线性空间 $V$ 的**线性变换的值域和核**都是 $V$ 的**线性子空间**。

> [!caution] 值域的联系
> 对于矩阵, 其**值域**是**列向量的线性组合**

定义: **象子空间的维数dimR(T)称为线性变换 T 的**<b><mark style="background: transparent; color: blue">秩</mark></b>, **核子空间的维数** $\dim N(T)$ 称为线性变换 T 的零度或者线性变换 $T$ 的<b><mark style="background: transparent; color: red">亏</mark></b>, 记为 $\text{null}(T)$ 
...
1. 象空间是线性空间基所生成的子空间， 即
$$R(T) = span\left\{ T(\alpha_1), T(\alpha_2), \dots T(\alpha_n) \right\}$$
2. 线性变换的秩等于其象空间的秩和核空间的秩之和总是等于全空间的维数n($rankT + nullT = n$)
$$\dim  R(T)+ \dim N (T) = n$$
### (2) 线性变换的运算
...
1. 结合律: $(T_1 T_2) T_{3} = T_{1}(T_{2}T_{3})$ 
2. 分配律: $(k + l) T = kT + l T$
3. 数乘结合律: $(kl) T = k(lT)$
4. $1 T = T$
5. (注意:不满足交换律: $T_1 T_2 \neq T_2 T_1$) 
...
> [!NOTE] 说明
> 容易证明, $T_1 + T_2$ 是线性变换。$kT$ 数乘也是 $V$ 上的线性变换; 线性变换的乘积也是线性变换, 而产生一个新的空间。
> 

线性变换的多项式:
设 $n$ 为正整数, T 是V的线性变换, 定义 T 的 n 次幂为:
$$\Large \boxed{T^{n} = TTTT\dots T }$$
并定义 $T$ 的**零次幂**为 $T^0 = T_e$，而当 $T$ 可逆时, 定义 $T$ 的负整数幂为:
$$T^{-n} = (T^{-1})^{n}$$

需要注意的是, 线性变换乘积的指数法则不成立: 即一般有: 
$$(ST)^{n} \neq  S^{n} T^{n}$$

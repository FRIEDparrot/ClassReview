...
1. $T_1 + T_2$ 是线性变换。
2. $kT$ 数乘也是 $V$ 上的线性变换; 
3. V 上<mark style="background: transparent; color: red">所有线性变换的集合</mark>，产生一个新的线性空间， 称为线性空间 $V$ 的<b><mark style="background: transparent; color: blue">同态</mark></b>, 并记为
$$Hom(V,V)$$
...
而 T 是线性变换, 则称其为**线性变换 $T$ 的多项式**
需要说明的是, 同一线性变换的多项式相乘是可以变换的, 有:
$$f(T) g (T) = g(T) f(T)$$
> [!NOTE] 重点考察内容

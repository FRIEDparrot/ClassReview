...
<b><mark style="background: transparent; color: orange">椭圆范数(加权范数)</mark></b>: 设$A$ 是任意的 $n$ 阶==**实对称正定矩阵**==, 且有列向量 $x \in R$, 则函数
...
<b><mark style="background: transparent; color: orange">定理(矩阵的从属范数)</mark></b>: 称为由向量范数导出的矩阵范数, 即从属范数。已知 $C^m$ 和 $C^n$ 上的同类向量范数 $||\centerdot||$ , 此时设有 $A \in C^{m \times n}$, 则函数

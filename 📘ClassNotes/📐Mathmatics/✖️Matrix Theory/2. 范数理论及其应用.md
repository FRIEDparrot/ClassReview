...
$$ \lim_{k \rightarrow \infty} \xi_{i}^{(k)}  = \xi_{i} \tag{1.1.1} $$
则记 $x = (\xi_1, \xi_2, \dots \xi_n)$, 则称向量序列 $\left\{ x_i^{(k)}\right\}$ 有极限 $x$, 或者称 $x^{(k)}$ 收敛于 $x$,  简称 $x_i$ 收敛, 记为:
$$ \lim_{k \rightarrow   \infty} x^{(k)}  =  x  \quad  \text{or} \quad x_{k}\rightarrow  x \tag{1.1.2} $$
而**不收敛的向量序列**称为是**发散**的。
...
2. 齐次性: 
$$ ||ax|| = |a|||x||\qquad  a \in  K, x \in  V \tag{1.1.3} $$
3. 三角不等式:
$$ ||x + y|| \leq ||x || +|| y|| \qquad  x, y \in  V \tag{1.1.4} $$
...
$$ ||x||_{\infty} = \max_{i}  |\xi_i| \tag{1.2.5} $$

<b><mark style="background: transparent; color: blue">椭圆范数(加权范数)</mark></b>: 设$A$ 是任意的 $n$ 阶==**实对称正定矩阵**==, 且有列向量 $x \in R$, 则函数
$$ ||x||_{A} =  (x ^{T} A x)^{\frac{1}{2}} \tag{1.2.6} $$
称为**加权范数或者椭圆范数**, 也称为 **Hibert-Schmidt 范数**;

补充: 对于任意的向量 a, b，均满足 Holder 不等式:
$$ \boxed{\Large \sum_{i = 1}^{n}|a_{i} b_{i}| \leq   \left(\sum_{i = 1}^{n}|a_{i}|^{p}\right)^{\frac{1}{p}}  \left(\sum_{i = 1}^{n} |b_{i}|^{q} \right)^{\frac{1}{q}}} \tag{1.2.7} $$
其中满足: 
$$ \frac{1}{p} + \frac{1}{q} = 1 , p > 1 , q  > 1 \tag{1.2.8} $$
而可以利用上式证明, $p$ 范数**满足对应的三角不等式**: 
$$ \Large\boxed{ \left(\sum_{i = 1}^{n} |\xi_{i} + \eta_{i}|\right)^{\frac{1}{p}} \leq  \left(\sum_{i = 1}^{n} |\xi_{i} |^{p}\right)^{\frac{1}{p}} +  \left(\sum_{i = 1}^{n} |\eta_{i} |^{p}\right)^{\frac{1}{p}} } \tag{1.2.9} $$

<b><mark style="background: transparent; color: orange">连续函数的积分范数</mark></b>(广义的连续函数的 p 范数): 
$$ ||f(t)||_{p}   =  \left[\int_{a}^{b}  \left| f(t) dt\right|^{p} \right]^{\frac{1}{p} } \tag{1.2.10} $$
连续函数区间上的最大值定义为连续函数的 $\infty$ 范数:
$$ ||f(t)||_{\infty}  = \max_{t \in [a,  b]} \left|f() \right| \tag{1.2.11} $$
...
$$ ||x||_{p}  = ||\alpha||_{p} \qquad  (1 \leq  p < +\infty) \tag{1.2.12} $$
...
$$ \Large\boxed{c_{1} ||x||_{\beta}    \leq   ||x||_{\alpha}  \leq   c_{2}  ||x||_{\beta}, \qquad  \forall  x \in  V} \tag{1.3.1} $$
...
定理2.2: $C^n$ 中的向量序列
$$ x^{(k)} =  (\xi_{1}^{(k)}, \xi_{2}^{(k)}, \dots  , \xi_{n}^{(k)})\qquad (k = 1,2,3\dots  ) \tag{1.3.2} $$
...
$$ ||AB|| \leq  ||A|| \quad ||B|| \quad   (B \in  C^{n \times  l}) \tag{2.1.1} $$
则称 $||A||$ 为 A 的**矩阵范数**. 
...
$$ A^{(k)} \rightarrow  A \tag{2.1.2} $$
<b><mark style="background: transparent; color: orange">有如下性质</mark></b>:
$$ A^{(k )}\rightarrow A\quad   \leftrightarrow  \quad  ||A^{(k)} -  A ||\rightarrow  0 \tag{2.1.3} $$
$$ \left|\space ||A|| - ||B|| \space  \right| \leq  ||A - B|| \tag{2.1.4} $$
$$ ||A^{(k)} - A|| \rightarrow  0  \leftrightarrow   ||A^{(k)}||  \rightarrow  ||A|| \tag{2.1.5} $$
称为**矩阵范数的连续性**

常见的矩阵范数包括:
**矩阵的 $m_1$ 范数**:
$$ ||A||_{m1} =  \sum_{i = 1}^{n}\sum_{j = 1}^{n}  | a_{ij} | \tag{2.1.6} $$
**矩阵的 $m_\infty$ 范数** 
$$ ||A||_{m \infty}  =   n \max_{i , j }  \left|a_{ij} \right| \tag{2.1.7} $$
**矩阵的 F 范数**(Frobenius范数): 
$$ \Large \boxed{||A||_{F} = \left( \sum_{i = 1}^{m} \sum_{j = 1}^{n}  \left|a_{ij}^{2} \right|\right)^{\frac{1}{2}} =  \text{tr} (A^{H} A )^{\frac{1}{2}}} \tag{2.1.8} $$
有 $||A||_{F} = ||A^{H}||_{F}$, 另外有关系: <b><mark style="background: transparent; color: orange">F 范数的平方</mark></b>等于**其各个列向量的 2 范数平方之和**: 
$$ ||A||_{F}^{2} =  \sum_{i = 1}^{n} ||a_{i}||_{2}^{2} \tag{2.1.9} $$
> [!CAUTION] 注意
> 更加常用的公式是
...
$$ \Large\boxed{||Ax||_{V} \leq  ||A||_{M} ||x||_{V}\qquad (\forall  A   \in  C^{(m \times  n)},  \forall x  \in  C^{n})   } \tag{2.2.1} $$
...
$$ \boxed{\Large||x||_{V} = ||x y^{H}||_{M}} \tag{2.2.2} $$
...
$$ \boxed{\Large||A|| = \max_{||x|| = 1} ||Ax||} \tag{2.2.3}$$
...
$$ || I y ||\leq    ||  I  ||\quad  ||y|| \overset{y \neq 0}{\longrightarrow}  ||I ||\geq  1 \tag{2.2.4} $$
...
1. 从属 1 范数:
$$ ||A||_{1} = \max_j \sum_{i = 1}^{n}\left|a_{ij} \right| \tag{2.2.5} $$
2. 针对 2 范数:
$$ ||A||_{2} =  \sqrt{\lambda_{1}} \tag{2.2.6} $$
其中 $\lambda_{1}$ 是矩阵 $A^H A$ 的最大特征值; 
3. 针对 $\infty$ 范数: 称为<b><mark style="background: transparent; color: orange">行和范数</mark></b> 
$$ ||A||_{\infty} =  \max_{i} \sum_{j = 1}^{n} |a_{ij}| \tag{2.2.7} $$
...
$$ ||F|| \leq  \frac{||A^{-1}B||}{1 - ||A^{-1} B||} \tag{3.1.3} $$
3. 满足:
$$ \frac{||A^{-1}  - (A + B)^{-1}||}{||A^{-1}||} \leq  \frac{||A^{-1} B||}{1 - ||A^{-1}B||} \tag{3.1.4} $$

其中,我们取 
$$ \Large\boxed{\text{cond} (A) = ||A|| \space ||A^{-1}||\quad  d_{A}= ||\delta A||\space ||A||^{-1}} \tag{3.1.5} $$
称$\text{cond}(A)$为**矩阵的条件数**, 衡量了**矩阵的病态程度**, 利用上述结论可得到当有: 
$||A^{-1} || \space ||\delta A|| < 1$ ($||A^-1 B|| \rightarrow ||A^{-1}||\space ||\delta A||$)时, 满足:
$$ ||I - (I + A^{-1} \delta A)^{-1}|| \leq    \frac{d_{A}\text{cond(A)}}{1 - d_{A } \text{cond}(A)} \tag{3.1.6} $$
$$ \frac{||A^{-1} -  (A + \delta  A)^{-1 }||}{||A^{-1}||} \leq  \frac{d_{A}\text{cond}(A)}{1 - d_{A}\text{cond} (A) } \tag{3.1.7} $$
一般而言，**条件数越大, 则 $(A + \delta A)^{-1}$ 和 $A^{-1}$ 的误差越大**。

### (2) 矩阵的谱半径及其性质
矩阵 $A \in C^{n \times n}$ 的  n 个特征值为 $\lambda_1, \lambda_2, \dots \lambda_n$, 则称:
$$ \boxed {\Large \rho (A) = \max_{i} \left| \lambda_{i}\right|} \tag{3.2.1} $$
称为 A  的**谱半径** 

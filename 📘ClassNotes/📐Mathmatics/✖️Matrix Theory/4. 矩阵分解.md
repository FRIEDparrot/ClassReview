...
\end{matrix}\right]  = D U \tag{1.1.4}
$$
此时我们有:
$$ A =( L_{1}^{-1} L_{2}^{-1} \dots  L_{n}^{-1} ) DU  = LDU \tag{1.1.5} $$
其中有关系(<mark style="background: transparent; color: red">不要顺序记混!!!</mark>)
$$ \Large\boxed{L = L_{1}^{-1} L_{2}^{-1} \dots  L_{n}^{-1}} \tag{1.1.6} $$
...
$$ D = \text{diag}(d_{1}, d_{2},\dots d_{n}) \tag{1.2.1} $$
...
\end{matrix}\right] \tag{1.3.5}
$$
另外由上面两个分解可以得到 $A$ 行列式的计算公式:
$$ \det A = \det A_{11}  \cdot  \det (A_{22}- A_{21} A_{11}^{-1} A_{12}) \tag{1.3.6} $$
$$ \det A = \det A_{22} \cdot  \det( A_{11}-A_{12} A_{22}^{-1} A_{21}) \quad (A_{22}可逆) \tag{1.3.7} $$
<b><mark style="background: transparent; color: orange">定理</mark></b>: 设 $A \in   R^{m \times n},  B \in R^{n \times  m}$, 则有:
$$ \det  (I_{m} \pm  AB ) = \det  (I_{n} \pm BA ) \tag{1.3.8} $$
特别地, 对于 $a \in R^{n \times 1}, b \in R^{1 \times  n}$, 有关系:
$$ \det (I_{n} +  b a^{T}) = 1 + a^{T} b \tag{1.3.9} $$
...
$$ T_{ij} = T_{ij}(c,s) \tag{2.1.2} $$

Givens 矩阵的性质: 
1. 显然, **Givens 矩阵为正交矩阵**, 有 : 
$$ \Large\boxed{T_{ij}(c,s) \cdot  T_{ij}^{T} = I \qquad   T_{ij}^{-1}(c,s) =  T_{ij}^{T}(c,s) = T_{ij}(c, -s)} \tag{2.1.3} $$
2. Givens 矩阵秩为1，有:
$$ \det [T_{ij}(c,s)] =  1 \tag{2.1.4} $$
...
$$ \eta_i = c\xi_i + s\xi_{j}\qquad    \eta_{j}= -s \xi_{i} + c  \xi_{j}\qquad \xi_{k} =  \eta_{k} \tag{2.1.5} $$
此时当 $\xi_i^2 + \xi_j^2 \neq 0$ 时, 如果我们取 :
$$ c = \frac{\xi_{i}}{\sqrt{\xi_{i}^{2} + \xi_{j}^{2}}} \qquad s =  \frac{\xi_{j}}{\sqrt{\xi_{i}^{2}  + \xi_{j}^{2}}} \tag{2.1.6} $$
则容易算出: 
$$ \eta_{i} =  \sqrt{\xi_{i}^{2}  +   \xi_{j}^{2}} \qquad  \eta_{j}  = 0 \tag{2.1.7} $$
...
$$ \boxed{\Large H = I - 2 u u^{T}} \tag{2.1.9} $$
...
性质:
1. **正交对称性**
$$ H^{T} = H, \qquad H^{T}H= I \tag{2.1.10} $$
2. **对合性质, 自逆性质**
$$ H^{2}  =  I \qquad   H^{-1} = H  \qquad \left| H\right| = -1 \tag{2.1.11} $$
...
$$ H x =  |x| z \tag{2.1.13} $$
且有此时:
$$ \boxed{\Large H = I - 2 uu^{T} \qquad  u = \frac{x - |x|z}{\left|x - |x|z \right|}} \tag{2.1.14} $$
...
\end{matrix}\right]_{I(i,j)} =  H_{u} H_{v} = (I - 2 u u^{T} ) (I - 2 v v^{T}) \tag{2.1.15}
$$
其中:
$$ u = \left(1,\dots  \sin \frac{\theta}{4} , \dots , \cos \frac{\theta}{4} \dots  1\right)\qquad  v = \left(1,\dots  \sin -\frac{\theta}{4} , \dots , \cos \frac{\theta}{4} \dots  1\right) \tag{2.1.16} $$
...
\end{matrix}\right] \tag{2.2.4}
$$
其中满足: 
$$ c = \frac{\xi_{i}}{|\sqrt{|\xi_{i}|^{2} +  \left| \xi_{j}\right|^{2}}}\qquad  s = \frac{\xi_{j}}{\sqrt{|\xi_{i}|^{2} + |\xi_{j}|^{2}}} \tag{2.2.5} $$
$m = n$, 则  $Q$ 为正交阵, 而 
...
$$ P = (e_{j_{1}}, e_{j_{2}}, \dots  e_{j_{n}}) \tag{3.2.1} $$
...
$$ N(A) =  \text{span} (v_{r + 1}, v_{r +2}, \dots  v_{n}) \tag{4.4} $$
$$ R(A) = \text{span}(u_{1}, u_{2}, \dots  u_{r}) \tag{4.5} $$
$$ A =  \sigma_{1} u_{1}v_{1}^{H} + \sigma_{2} u_{2} v_{2}^{H} + \dots  + \sigma_{r} u_{r} v_{r}^{H} \tag{4.6} $$
...
$$ \boxed{\Large  B = U^{-1} A V} \tag{4.1.1} $$
则称为**矩阵 A, B 正交相抵**

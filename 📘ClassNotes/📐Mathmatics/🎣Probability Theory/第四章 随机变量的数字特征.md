...
我们设$X$的**分布律**为: 
$$ P\{X= k\} = p_{k}\qquad k = 0, 1, 2 \dots \tag{1.1.1} $$
射击$N$次，其中**得$0$分的有$a_0$次, 得1分的有$a_1$次, 得2分的有$a_{2}$次**, 显然有$a_{0} + a_{1} + a_{2} = N$, 总分为随机变量$X$, 我们取<u>平均射击一次的得分</u>为随机变量$X$的**数学期望**: 
$$ X = \frac{a_{0} \times 0 + a_{1}\times 1 + a_{2}\times 2}{N} = \sum^{2}_{k=0}k \frac{a_{k}}{N} \tag{1.1.2} $$
...
$$ \boxed{E(x) = \sum^{\infty}_{k=1} x_{k} p_{k}} \tag{1.1.3} $$
...
$$ E(X) = \int_{-\infty}^{\infty} x f(x) dx \tag{1.1.4} $$
...
$$ \Large\boxed {E(Y) = E[g(x)] = \sum^{\infty}_{k=1} g(x_{k}) p_{k}} \tag{1.2.1} $$
2. 如果$X$是连续型随机变量, 概率密度为$f(x)$, 则有<mark style="background: transparent; color: red">(前提是右侧绝对收敛)</mark>
$$ \Large \boxed{E(Y) = E[g(X)] = \int_{- \infty }^{ + \infty }  g(x) f(x) dx} \tag{1.2.2} $$
3. 对于**二维随机变量, 有离散型和连续型的概率密度公式<mark style="background: transparent; color: red">(前提是右侧绝对收敛)</mark>**: 
$$ \boxed{E(Z) = \sum^{\infty}_{j=1} \sum^{\infty}_{i=1} g(x_{i}, y_{i}) p_{ij} \qquad  E(Z) = E[g(X,Y)] = \int_{-\infty}^{+ \infty} \int_{-\infty}^{+ \infty} g(x,y)f(x,y) dx dy} \tag{1.2.3} $$
`````ad-note
title: 笔记
collapse: open
...
$$ \Large\boxed{D(x) = E\{[X - E(X)]^{2}\}} \tag{2.1.1} $$
...
$$ \Large \boxed {D(X) = \sum^{\infty}_{k=1} [x_{k} - E(X)]^{2} p_{k} } \tag{2.2.1} $$
对于**连续型随机变量**, 有:
$$ \Large \boxed {D(x) = \int_{-\infty }^{+\infty} [x - E(x)]^{2}f (x) dx} \tag{2.2.2} $$
...
$$ \Large \boxed{D(X) = E(X^{2}) - [E(X)]^{2}} \tag{2.2.3} $$
...
$$ X^{*} = \frac{X - \mu}{\sigma} \quad \rightarrow  \quad E(X^{*}) = 0, D(X^{*}) = 1 \tag{2.3.1} $$
...
$$ P \{|X - \mu| \geq  \varepsilon\} \leq \frac{\sigma^{2}}{\varepsilon^{2}} \tag{2.3.2} $$
...
$$ E\{[X - E(x)] [Y - E(Y)] \} = 0 \tag{3.1} $$
此时，若$X,Y$不相独立, 则$E \{[X- E(X)][Y -E(Y)]\}\neq 0$, 因此我们定义上式为**随机变量的协方差** :
$$ \boxed{\Large \text{Cov} (X, Y) = E\{[X - E(X)][Y - E(Y)]\} = E(XY) - E(X) E(Y)} \tag{3.2} $$
并定义**随机变量$X$与$Y$的**<b><mark style="background: transparent; color: red">相关系数</mark></b>如下。
$$ \boxed{\rho_{XY} = \frac{\text{Cov}(X,Y)}{\sqrt{D(X)} \sqrt{D(Y)}}} \tag{3.3} $$
...
$$ \Large\boxed{E(X^{k}) \qquad k = 1, 2 \dots} \tag{4.1.1} $$
**原点矩简称$k$阶矩**
2. <mark style="background: transparent; color: red">中心矩</mark>: 设$X, Y$为随机变量，若有:
$$ \Large \boxed{E\{[X - E(X)]^{k}\}\qquad   k = 1,2 \dots} \tag{4.1.2} $$
...
$$ c_{11} = E\{[X_{1} - E(X_{1}) ]^{2}\}\qquad c_{12} = E\{[X_{1} - E(X_{1}) ] [X_{2}- E(X_{2})]\} \tag{4.2.1} $$
$$ c_{21} = E\{[X_{2} - E(X_{2}) [X_{1} - E(X_{1})]] \}\qquad c_{22} = E\{[X_{2} - E(X_{2}) ]^{2}\} \tag{4.2.2} $$
并将其组装得到$n$维随机变量的<mark style="background: transparent; color: red">协方差矩阵</mark>, 从而有:
$$
C = \left(\begin{matrix}
c_{11} & c_{12}  \\  c_{21} & c_{22}
\end{matrix} \right) \tag{4.2.3}
$$
...
\end{matrix}\right]^{T} \tag{4.3.1}
$$
则$n$维正态随机变量的概率密度定义为:
$$ f(x_{1}, x_{2},\dots  x_{n})  = \frac{1}{\sqrt{2\pi}^{n} (\det C )^{\frac{1}{2}}} \exp \left\{-\frac{1}{2} (X - \mu )^{T} C^{-1} (X- \mu)\right\} \tag{4.3.2} $$
`````ad-note
title: $n$维随机变量的性质
collapse: open
...
$$ \Large\boxed{\mathcal{N} (x|\mu, \Sigma) = \frac{1}{(2\pi)^{\frac{n}{2}} |\Sigma|^{\frac{1}{2}}}\exp \left\{-\frac{1}{2}(x-\mu)^{T}\Sigma^{-1} (x - \mu)\right\}} \tag{4.4.1} $$
其中, $\Sigma$ 为协方差矩阵, 并定义为:
$$ \Large\boxed{\Sigma = E[(x- \mu)(x- \mu)^{T}]} \tag{4.4.2} $$

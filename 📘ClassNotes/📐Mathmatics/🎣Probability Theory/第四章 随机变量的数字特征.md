...
由[[📘ClassNotes/📐Mathmatics/🎣Probability Theory/补充部分/二维正态分布的协方差和相关系数.pdf|二维正态分布的协方差和相关系数.pdf]], 对于标准正态分布, 协方差为$Cov(X, Y) = \sigma_1 \sigma_2 \rho$, 相关系数为$\rho_XY = \rho$
`````
`````ad-note
title: 相关与相互独立的关系
collapse: open
...
$$E(X^{k} Y^{l})$$
存在, 则称为X$,Y$的$k + l$阶<mark style="background: transparent; color: red">混合矩</mark>(混合原点矩)。 相应地，定义$X,Y$的$k+ l$阶<mark style="background: transparent; color: red">混合中心矩</mark>为
$$E\{[X - E(X)]^{k} [Y - E(Y)]^{l} \}$$
`````ad-cite 
title: 矩和数字特征的关系
collapse: open 

...
且$\overline{X}$与$S$相互独立

**定理三**: 设$X_1, X_{2}, \dots X_{n}$是来自总体$N(\mu,\sigma^2)$的样本, $\overline{X}, S^2$分别是样本的均值和样本的方差, 则有: 
$$\frac{\overline{X} - \mu}{\frac{S}{\sqrt{n}}} \sim t(0,1)$$
...
且其中有:
$$S_{w}^{2} = \frac{(\overline{X} - \overline{Y}) - (\mu_{1} - \mu_{2})}{n_{1} + n_{2} - 2}\qquad S_{w} = \sqrt{S_{w}^{2}}$$

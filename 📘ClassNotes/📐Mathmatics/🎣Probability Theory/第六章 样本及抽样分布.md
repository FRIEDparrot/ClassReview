...
样本的方差: (实际上是每个样本方差和的加权平均, 但平均系数为$n-1$)
...
样本的$k$阶<mark style="background: transparent; color: red">原点矩</mark>:
$$A_{k} = \frac{1}{n}\sum^{n}_{i=1} X^{k}_{i}, \quad k = 1, 2\dots $$
样本的$k$阶<mark style="background: transparent; color: red">中心矩</mark>: 

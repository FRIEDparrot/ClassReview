...
$$ \boxed{P(X = x_{k}) = p_{k} , \quad  k = 1, 2,\dots} \tag{1.1.1} $$
且满足如下两个条件:
$$ p_{k}\geq 0, k = 1, 2, \dots \qquad \qquad \sum^{\infty}_{k=1} p_{k} =  1 \tag{1.1.2} $$
...
#### 1. 0-1分布(两点分布)
分布律为: 
$$ P(x = k) = p^{k}(1- p)^{1-k}\qquad \text{where } k = 0, 1 \tag{1.2.1} $$
...
$$ P(X = k) =C^{n}_{k} p^{k} q^{n-k} \qquad k = 0,1,\dots n \tag{1.2.2} $$
其中$n = 1$时， 二项分布退化为0-1分布

#### 3. 泊松分布
我们设随机变量的所有可能取的值为$0,1,2\dots$, 而取得各个值的概率分别为:
$$ \Large P\left\{X = k \right\} = \frac{\lambda^{k} e^{-\lambda}}{k !} \qquad k = 0,1,2\dots \tag{1.2.3} $$
...
$$ \sum^{\infty}_{k=0} \frac{\lambda^{k} e^{-\lambda}}{k!} = 1 \tag{1.2.4} $$
...
$$ \boxed{\lim_{n \rightarrow \infty} C_{n}^{k} p^{k} (1- p)^{n-k}  = \frac{\lambda^{k} e^{-\lambda}}{ k!}} \tag{1.2.5} $$
`````ad-caution 
title: 说明
collapse: open
...
$$ F(x) = P\{ X\leq x\}\qquad -\infty < x < + \infty \tag{1.3.1} $$
称为$X$的**分布函数**。分布函数描述了$X$落在任意区间的规律 显然有: 
$$ F(x_{2}) - F(x_{1}) =  P\{x_{1}  < X \leq  X_{2} \} \tag{1.3.2} $$
**离散型随机变量的分布函数**一般形式为: 
$$ F(x) = \sum_{x_{k} \leq x} P\{ X = x_{k}\} \tag{1.3.3} $$
...
$$ \boxed{F(x) = \int_{-\infty}^{x} f(t) dt} \tag{2.1.1} $$
此时， 称$X$为连续型随机变量，而$f(x)$为$X$的<mark style="background: transparent; color: red">概率密度函数</mark>，简称**概率密度**, 而$F(x)$是$X$的<u>分布函数</u>，此时有: 
$$ F(x_{2})- F(x_{1}) = \int_{x_1}^{x_2} f(x) dx\qquad f(x) = \lim_{\Delta x \rightarrow 0^{+}} \frac{F(x + \Delta  x) - F(x)}{\Delta x} = F'(x) \tag{2.1.2} $$
`````ad-note
title: 分布函数的性质
collapse: open
1. $f(x) \geq 0$
...
$$ P \{c < X \leq c + l \} = \int_{c}^{c+ l} \frac{1}{b- a} dx  = \frac{l}{b-a} \tag{2.2.3} $$
#### 2. 指数分布
...
$$ \Large \boxed {P\{ X \geq s + t |  X \geq  s\}  = P(X \geq  t)} \tag{2.2.6} $$
该式称为<b><mark style="background: transparent; color: blue">无记忆性</mark></b>,  表明总共能够使用$t$小时的概率和使用$s$小时之后, 还能使用$t$小时的概率相等，因而在可靠性分析中应用广泛。
#### 3. 正态分布
正态分布随机变量$X$的**概率密度**为: 
$$ \Large \boxed{f(x) = \frac{1}{\sqrt{2\pi}\sigma} e^{- \frac{(x - \mu)^{2}}{2\sigma^{2}}} \quad - \infty < x < \infty} \tag{2.2.7} $$
...
正态分布的**分布函数**为: 
$$ \Large \boxed{F(x) = \frac{1}{\sqrt{2\pi }\sigma } \int_{-\infty}^{x} e^{-\frac{(t - \mu)^{2}}{2\sigma^{2}}}dt} \tag{2.2.8} $$
...
$$ \Large\boxed{\phi(x) = \frac{1}{\sqrt{2\pi}} e^{- \frac{x^{2}}{2}} \qquad \Phi (x) = \frac{1}{\sqrt{2 \pi}}\int_{-\infty}^{x} e^{-\frac{t^{2}}{2}}dt} \tag{2.2.9} $$
...
#### 4. $\chi^{2}$分布 
我们取$X\sim N(0,1)$, 即有:
$$ f(X) = \frac{1}{\sqrt{2\pi}} e^{-\frac{x^{2}}{2}}\qquad -\infty < x  < +\infty \tag{2.2.10} $$
...
对于正态分布， 我们有: 
$1\sigma$区间: 
$$ P\{ \mu  - \sigma \leq  X \leq  \mu + \sigma\} = 0.6826 \tag{2.3.1} $$
$2\sigma$区间:
$$ P\{\mu - 2\sigma \leq X \leq  \mu + 2\sigma\} = 0.9544 \tag{2.3.2} $$
$3\sigma$区间:
$$ P\{ \mu  - 3 \sigma \leq X \leq \mu + 3\sigma\} = 0.9974 \tag{2.3.3} $$

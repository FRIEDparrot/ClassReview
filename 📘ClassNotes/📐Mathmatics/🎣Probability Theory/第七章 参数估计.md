...
$$ \mu_{l} = E(X^{l}) = \int_{-\infty}^{+\infty}x^{l} f(x, \theta_{1}, \theta_{2}, \dots \theta_{k}) dx \qquad x\text{连续型} \tag{1.1.1} $$
$$ \mu_{l} = E(X^{l}) = \sum_{x \in R_{x}}  x^{l}p (x, \theta_{1}, \theta_{2}\dots \theta_{k})\qquad  x离散型 \tag{1.1.2} $$
基于样本的$k$阶矩, 有$\mu_{l}= A_{l}$, 
$$ A_{l} = \frac{1}{n}\sum^{n}_{i=1}  X_{i}^{l} \overset{参数估计}{\Longrightarrow} \boxed{\Large \mu_{l} = A_{l}} \tag{1.1.3} $$
我们将<mark style="background: transparent; color: red">待估计参数</mark>$\theta_1, \theta_{2}, \dots$写成$\mu$的函数, 并使用$A_{l}$代替$\mu_{l}$, 则有:
$$ \Large \boxed{\hat{\theta}_{i} = \theta_{i} (A_{1}, A_{2} , \dots A_{k})\quad i = 1, 2\dots k} \tag{1.1.4} $$
...
$$ \prod_{i=1}^{n} p(x_{i}, \theta) \tag{1.2.1} $$
> [!NOTE] 说明
> 显然，取到 $x$ 的概率等于取到其各个分量的条件概率乘积 :  
> $$ p(x|\theta) =  \prod_{i=1}^{n} p(x_{i}, \theta)\qquad \theta \in \Theta$$

设$x_1, x_2, \dots$是一个样本的观察值, 则**样本恰好取到观察值的概率**为 : 
$$ L(\theta) = L(x_{1}, x_{2}, \dots x_{n},  \theta) = \prod_{i=1}^{n} p(x_{i}, \theta)\qquad \theta \in \Theta \tag{1.2.2} $$
...
$$ \boxed{L(x_{1}, x_{2}, \dots  x_{n}, \hat{\theta}) = \max_{\theta \in \Theta} L(x_{1,}x_{2}, \dots x_{n}, \theta)} \tag{1.2.3} $$
...
$$ \prod_{i=1}^{n} f(x_{i}, \theta) \tag{1.2.4} $$
此时取得的随机点$(X_{1}, X_{2}, \dots X_{n})$落在点$(x_1, x_2, \dots x_n)$邻域内的概率如下, 我们只需要考虑最大值: 
$$ L(\theta) = L (x_{1}, x_{2}, \dots x_{n}, \theta) = \prod_{i=1}^{n} f(x_{i}, \theta) dx_{i}\quad \rightarrow \quad  L (\hat{\theta}) = \max L (x_{1}, x_{2},\dots \theta) \tag{1.2.5} $$
这个函数的值随$\theta$变化, 为了方便我们利用ln做运算后求导, 则最大似然估计$\hat{\theta}$也可以利用<mark style="background: transparent; color: red">对数似然方程组</mark> 
$$ \boxed{\frac{d}{d\theta} \ln  L (\theta) = 0} \tag{1.2.6} $$
当分布中含有多个参数$\theta_{1}, \theta_{2}, \dots \theta_{k}$时, 则可以令
$$ \frac{\partial }{\partial \theta_{i}} \ln L  = 0\qquad i = 1, 2 \dots k \tag{1.2.7} $$
...
\end{cases} \qquad \qquad  P_{\text{not collapse}} = \int_{t_{m}}^{\infty} \frac{1}{\theta} e^{- \frac{t}{\theta}} dt \tag{2.2.1}
$$
...
$$ \Large P(t \in  (t_{i}, t_{i} + dt_{i}]) = \frac{1}{\theta} e^{- \frac{t_{1}}{ \theta}}dt \tag{2.2.2} $$
使用**最大似然估计法确定$\theta$ 的取值**, 则<mark style="background: transparent; color: red">有m个产品失效的概率</mark>为前m个产品的失效概率和后n-m个产品的不失效概率相乘 :
$$ P_{前m个产品失效} =\prod_{i=1}^{m} \frac{1}{\theta} e^{- \frac{t_{i}}{\theta}} d t_{i}\qquad  P_{另外的n-m个产品不失效} =C_{n}^{m} \left(e^{- \frac{t_{m}'}{\theta}} \right)^{n-m} \tag{2.2.3} $$
则有m个产品在对应时间失效的概率为:
$$ \Large P =C_{n}^{m} \frac{1}{\theta^{m}} e^{- \frac{t_{1} + t_{2} + \dots + t_{m} + (n-m)t_{m}}{\theta}} dt_{1} dt_{2} \dots  dt_{m} \tag{2.2.4} $$
忽略常数因子不影响概率估计, 因而有<mark style="background: transparent; color: red">对数似然函数</mark>忽略前面的系数 :
$$ \ln L(\theta) = -m \ln \theta - \frac{1}{\theta} [t_{1} + t_{2}  + \dots + t_{n}] \tag{2.2.5} $$
此时求导并令导数为0, 得到<mark style="background: transparent; color: red">基于定数截尾实验的最大似然估计值</mark>: 
$$ \Large\boxed{\hat{\theta}= \frac{t_{1}+ t_{2} + \dots + t_{m} + (n-m) t_{m}'}{m} = \frac{s(t_{m})}{m}} \tag{2.2.6} $$
...
$$ \Large \boxed{\hat{\theta} = \frac{t_{1} + t_{2}+ \dots + t_{m} + (n-m)t_{0}}{m} = \frac{s(t_{0})}{m}} \tag{2.3.1} $$
## 三、 估计量的评选标准
### (1) 无偏性
首先要求$\hat{\theta} = \hat{\theta}(X_1, X_2, \dots X_{n})$的数学期望$E(\hat{\theta})$存在, 且对于任意的$\theta \in \Theta$有: 
$$ E(\hat{\theta}) = \theta \tag{3.1.1} $$
...
$$ E(\overline{X}) = \mu\quad E(S^{2}) = \sigma^{2} \tag{3.1.2} $$
...
$$ \boxed{D(\hat{\theta}_{1}) \leq  D(\hat{\theta}_{2})} \tag{3.2.1} $$
...
$$ \lim_{n\rightarrow \infty} P\left\{|\hat{\theta} - \theta| < \varepsilon \right\} =1 \tag{3.3.1} $$
...
$$ P\left\{ \underline{\theta}(X_{1}, X_{2} ,\dots X_{n})< \theta < \overline{\theta}(X_{1}, X_{2},\dots ,  X_{n})\right\}  \geq  1- \alpha \tag{4.1.1} $$
...
$$ \boxed{\left(\overline{X}  \pm  \frac{\sigma}{\sqrt{n}} z_{\frac{\alpha}{2}}\right)} \tag{5.1.1} $$
...
$$ \left(\overline{X}\pm \frac{S}{\sqrt{n }}t_{\frac{\alpha}{2}}(n-1 )\right) \tag{5.1.2} $$
其中$\frac{\overline{X} - \mu}{S/\sqrt{n}}$为枢轴量, 即其分布与$\mu$无关, t是t分布的上$\frac{\alpha}{2}$分位点, 同时我们可以推导出方差估计: 
$$ \left(0, \frac{(n-1)}{z_{1-\alpha}} S^{2}\right) \tag{5.1.3} $$
...
1) $\mu_1, \mu_2$均已知时, 有:
$$ (\overline{X}  - \overline{Y}  \pm z_\frac{\alpha}{2} \sqrt{\frac{\sigma_{1} ^{2}}{n_{1}} + \frac{\sigma_{2}^{2}}{n_{2}}}) \tag{5.2.1} $$
2) $\sigma_1^2  = \sigma_2^2  = \sigma^2$未知时, 有$\mu_1 - \mu_2$的1-$\alpha$置信区间为:
$$ (\overline{X} - \overline{Y} \pm t_{\frac{\alpha}{2}} (n_{1} + n_{2} - 2 ) S_{w}\sqrt{\frac{1}{n_{1}} + \frac{1}{n_{2}}})\qquad S_{w}^{2} = \frac{(n_{1} - 1)S_{1}^{2} + (n_{2} -1) S_{2}^{2}}{n_{1} + n_{2} - 2} \tag{5.2.2} $$
#### 2) 方差比的置信区间
...
$$ \frac{S_{1}^{2}}{S_{2}^{2}}/{\frac{\sigma_{1}^{2}}{\sigma_{2}^{2}}}\sim F(n_{1}-1, n_{2} - 1) \tag{5.2.3} $$
因此得到方差比的置信区间估计:
$$ \left(\frac{S_{1}^{2}}{S_{2}^{2}} \frac{1}{F_\frac{\alpha}{2}(n_{1} - 1 , n_{2}-1)}, \frac{S_{1}^{2}}{S_{2}^{2}}\frac{1}{F_{1- \frac{\alpha}{2}} (n_{1} - 1, n_{2} - 1)} \right) \tag{5.2.4} $$

## 六、0-1分布参数的区间估计
对于一个来自(0-1)分布总体的样本, 且$X$的分布律为
$$ f(x; p)  = p^{x} (1- p)^{1-x} \qquad x = 0,1 \tag{6.1} $$
则$p$的置信水平为$1-\alpha$的置信区间为$(p_1, p_2)$, 其中有
$$ \boxed{p_{1} = \frac{1}{2a}(- b - \sqrt{b^{2} - 4ac})\qquad p_{2} = \frac{1}{2a}(-b + \sqrt{b^{2} - 4ac})} \tag{6.2} $$
$$ \boxed{a = n + z_{\frac{\alpha}{2}}^{2}\qquad  b = -\left(2n\overline{X}  + z_{\frac{\alpha}{2}}^{2}\right) \qquad  c = n\overline{X}^{2}} \tag{6.3} $$

## 七、单侧置信区间估计
### (1) 置信区间的定义
对于给定值$\alpha \in (0,1)$, 若由样本$X_1, X_2, \dots X_{n}$确定的统计量$\underline{\theta}= \underline{\theta}(X_1, X_2, \dots X_{n})$, 对于任意的$\theta \in \Theta$满足: 
$$ P\left\{\theta > \underline{\theta} \right\}  \geq  1-\alpha \tag{7.1.1} $$
此时称$\underline{\theta}$ 为$\theta$ 的置信水平为$1 - \alpha$的<mark style="background: transparent; color: red">置信下限</mark>， 如果有
$$ P \left\{\theta < \overline{\theta} \right\} \geq  1-\alpha \tag{7.1.2} $$
...
$$ \frac{\overline{X} - \mu}{S\sqrt{n}}   \sim  t(n -1 ) \tag{7.2.1} $$
因此根据$t$分布, 则有关系:
$$ P\left\{ \frac{\overline{X} - \mu }{S/\sqrt{n}}< t_{\alpha}(n-1)\right\}    =1- \alpha \tag{7.2.2} $$
代换得到:
$$ P\left\{\mu >\overline{X} -\frac{S}{\sqrt{n}} t_{\alpha}(n-1) \right\} = 1 - \alpha \tag{7.2.3} $$
因此得到$1 - \alpha$置信下限为$\frac{S}{\sqrt{n}}t_{\alpha}(n-1) + \overline{X}$ 
同样地, 可以得到: $1- \alpha$置信上限为:
$$ \frac{S}{\sqrt{n}} t_{1- \alpha}(n-1) + \overline{X} \tag{7.2.4} $$
### (3) 正态总体方差的置信区间
我们仍然利用 
$$ \frac{(n-1) S^{2}}{\sigma^{2}} \sim \chi ^{2}(n-1) \tag{7.3.1} $$
得到关系: 
$$ \left\{ \sigma^{2} < \frac{ (n-1 )S^{2}}{\chi^{2}_{1-\alpha} (n-1)}\right\} = 1 - \alpha \tag{7.3.2} $$

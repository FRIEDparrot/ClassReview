## 一、微分方程的基本概念
#### (1) 微分方程基本概念
一般地，凡是表示未知数，未知函数的导数和自变量之间的关系的方程，都叫做微分方程。
其中，微分方程中最高阶导数项的阶次，称为微分方程的阶，例如，
$$x^3 y''' +x^2 y - 4xy' = 3x$$
...
微分方程的解是解出满足微分方程的函数
则$y = \varphi(x)$为微分方程$(1-11)$在区间$I$上的解

注意： 微分方程的解中含有任意常数，且任意常数的个数和微分方程的阶数相同
...
[[第七章 微分方程 2023-01-31 21.39.26.excalidraw|推导过程]]
得到：
...
\end{matrix}
\right| =0$$
即$\frac{a_1}{a} = \frac{b_1}{b}$
则令$\frac{a_1}{a} =\frac{b_1}{b} = \lambda$, 则可以对下方进行代换：
$$\frac{dy}{dx} = \frac{ax + by +c}{\lambda (ax + by) + c_1}$$
此时，引入新的变量 $v =ax+ by$
$$\frac{dv}{dx} = a + b \frac{dy}{dx} = a + b\frac{v + c}{\lambda v + c_1}  \quad \text{then} \quad \frac{1}{b}(\frac{dv}{dx} -a) = \frac{v+c}{\lambda v + c_1}$$
即有可分离变量的方程：
...
我们做代换$\to$ 使用**常数变易法**(将C换成未知的$u(x)$)
$$y = ue^{-\int P(x) dx}$$
有[[第七章 微分方程 2023-01-31 23.52.04.excalidraw|注意事项]]
$$\frac{dy}{dx} = u' e^{-\int P(x)dx} -u  P(x)\space e^{-\int P(x)dx}$$
代入上式(4.1)，有:
$$u' e^{-\int P(x)dx} + \left[-uP(x)\space e^{-\int P(x)dx} + P(x)\cdot  ue^{-\int P(x)dx} \right]s= Q(x)$$
其中方括号内为0，则

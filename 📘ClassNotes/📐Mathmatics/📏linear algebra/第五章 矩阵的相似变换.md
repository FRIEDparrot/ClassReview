...
$$ \Large \boxed {(A - \lambda I)x = 0} \tag{1.1.1} $$
...
$$ \sum_{i=1}^n \lambda_i=\text{tr}(A),  \qquad \text{tr}(A) = a_{11} + a_{22} +... + a_{nn} \tag{1.2.2} $$
其中 $\lambda_i$ 表示第 $i$ 个特征值。
#### 3. 特征值与积
一个 $n$ 阶方阵 $A$ 的所有特征值之积等于其行列式: 
$$ \prod_{i=1}^n \lambda_i=|A| = \det(A) \tag{1.2.3} $$
`````ad-note
title: 推论
collapse: open
...
`````
设一个多项式为: 
$$ f(x)=\sum_{i=0}^mc_ix^i = c_0 + c_1 x + ... + c_{s-1}x^{s-1} + c_s x^s \tag{1.2.4} $$
**矩阵多项式**是将一个矩阵中的元素都替换成多项式得到的矩阵。定义上多项式的<mark style="background: transparent; color: red">矩阵多项式</mark> $f(A)$ 为: 
$$ \boxed{f(A)=\sum_{i=0}^mc_iA^i=c_0I_n+c_1A+c_2A^2+\cdots+c_mA^m} \tag{1.2.5} $$
`````ad-caution
title: 注意
collapse: open
...
$$ f(A)g(B)=\left(\sum_{i=0}^ma_iA^i\right)\left(\sum_{j=0}^nb_jB^j\right)=\sum_{i=0}^m\sum_{j=0}^na_ib_jA^iB^j \tag{1.2.6} $$
...
$$ \det  (\lambda  I - A) = \prod^{n}_{i=1} \det   (\lambda I_{i}- A_{i}) \tag{1.2.7} $$
此条根据行列式的性质容易证明
...
$$ |A| = |B| = \prod_{i=1}^{n} \lambda_{i} \tag{2.2.1} $$
4. 两个矩阵具有<mark style="background: transparent; color: red">相同的迹</mark>， 即有: 
$$ \sum^{n}_{i=1}  a_{ii}  = \sum^{n}_{i=1}  b_{ii} = \text{tr}(A) = \text{tr} (B) \tag{2.2.2} $$
...
\end{matrix}\right] = \Lambda \tag{2.3.1}
$$

...
$$\alpha = (a_1, a_2 , ... a_n)$$
称为$n$维**向量**， 其中数$a_j$为向量$\alpha$的第$j$个分量(或坐标)
...
为向量$\alpha, \beta$的**内积**。
显然有: <mark style="background: transparent; color:red">对于n维列向量</mark>$\alpha, \beta$, 则有其内积公式：
...
为**向量的模(或者称为范数/长度)**，范数为1的向量称为**单位向量**， 我们也可以通过$\vec{n} = \frac{\vec{\alpha}}{||\alpha||}$进行单位化向量
...
**定义**: 设$\alpha_1, \alpha_2,... \alpha_n$均为$n$维向量，如果存在<mark style="background: transparent; color: red">一组不全为0的数</mark>$k_1,k_2, ...k_n$， 使得
$$\Large\boxed{k_1 \alpha_1 +  k_2 \alpha_2 + ... + k_n\alpha_n = 0}$$
则称$\alpha_1, \alpha_2,...\alpha_n$线性相关，如果上式仅有在$k_i$等于0时才成立, 则称向量组$\alpha_i(i=1,2,..m)$线性无关， 即可以<u>判断其次线性方组的非零解来判断线性相关</u>
...
### (2) 线性相关的判别定理
**定理**: 向量组$\alpha_1, \alpha_2,...\alpha_m(m\geq 2)$线性相关的充要条件是其中<mark style="background: transparent; color: red">至少有一个向量</mark>可以由其余$m-1$个向量线性表示
**推论**: 两个向量线性相关的充要条件是其对的分量成比例
**定理:** 若向量组$\alpha_1, \alpha_2, ...\alpha_m$线性无关，而$\alpha_1, \alpha_2, ...\alpha_m,\beta$线性相关，则$\beta$可以唯一使用$\alpha_1, \alpha_2, ...\alpha_m$线性表示
**定理:** 如果<u>向量组的部分向量线性相关， 则这个向量组线性相关</u>
**推论1:**  含有零向量的向量组线性相关
...
则称$\alpha_1, \alpha_2, ... \alpha_r$为向量组秩的一个<mark style="background: transparent; color: red">极大线性无关向量组</mark>，简称<mark style="background: transparent; color: red">极大无关组</mark>，并称$r$为<mark style="background: transparent; color:red">向量组的秩</mark>，并记为$\text{rank} A$，规定仅含零向量组的秩为0, 没有极大无关组
...
如果$\alpha_1, \alpha_2,...$和$\beta_1, \beta_2,...$中， 每个向量均可以相互线性表示， 则称**向量组$T_1,T_2$等价**
**定理：向量与其任意一个极大无关组等价**
**推论： 向量组的任意两个极大无关组等价**
**定理:** 设有两个$n$维向量组
...
定理: 如果A 是正交矩阵， 则有:
$$A^{T}= A^{-1} $$
上式等同于$A$的<b><mark style="background: transparent; color: blue">行向量和列向量都是单位向量且两两正交</mark></b>
...
$$k_{21} = -\frac{[\alpha_2, \beta_1]}{[\alpha_1, \beta_{1]}}$$
...
当$x\in S, y\in  S, k\in R$时， 有$A(x + y) = Ax + Ay = 0$，以及$A(kx) = k(Ax) = 0$得知，$x + y\in S, kx\in S$， 由向量空间的定义可知，$S$是向量空间， 称为齐次下行方程组的**解空间**， 而$S$的基称为齐次线性方程组的**基础解系**
...
任意解向量均可以由上述$n-r$个向量线性表示，解空间的维数$\dim S = n-r$ 
结论： <mark style="background: transparent; color: yellow">解向量的个数等于线性方程组中的位置数的个数减去系数矩阵的秩</mark>
...
![[Excalidraw/第四章 线性方程组解的结构 2023-05-04 17.19.22|550]]

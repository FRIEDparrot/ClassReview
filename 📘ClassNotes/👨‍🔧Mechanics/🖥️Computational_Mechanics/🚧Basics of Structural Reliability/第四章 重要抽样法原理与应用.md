...
**失效概率的方差可以使用样本方差来近似**,  此时， 参考[[📘ClassNotes/👨‍🔧Mechanics/🖥️Computational_Mechanics/🚧Basics of Structural Reliability/第二章 蒙特卡洛模拟方法#二、求解失效概率的Monte Carlo方法与收敛性分析|可靠性灵敏度MCS方法中的推导过程]]得到重要抽样法对应的公式: 
...
## 二、重要抽样的灵敏度计算方法
### (1) 灵敏度计算公式
对于重要抽样, <b><mark style="background: transparent; color: blue">可靠性灵敏度</mark></b>的估计值可以确定为(推导参考[[📘ClassNotes/👨‍🔧Mechanics/🖥️Computational_Mechanics/🚧Basics of Structural Reliability/第二章 蒙特卡洛模拟方法#三、 求解可靠性局部灵敏度的分析与收敛分析|MCS可靠性局部灵敏度]]):
...
### (2) 相互独立正态分布的灵敏度求解
此时, 我们考虑$n$维相互独立的正态分布的情况。显然其中对于正态分布, 参考[[📘ClassNotes/👨‍🔧Mechanics/🖥️Computational_Mechanics/🚧Basics of Structural Reliability/第二章 蒙特卡洛模拟方法#(2) 相互独立的正态随机变量情况|相互独立的正态随机变量]]有: 
...
通过[[📘ClassNotes/👨‍🔧Mechanics/🖥️Computational_Mechanics/🚧Basics of Structural Reliability/推导部分/混合重要抽样的密度推导.pdf|混合重要抽样的密度推导.pdf]], 可以获取系统失效概率的样本估计公式, <mark style="background: transparent; color: red">实际上即为1到m个失效模式的</mark>$\alpha_{k}$之和:
...
> [!note] 变量不服从正态分布的情况
> 对于此种情况， 参考[[📘ClassNotes/👨‍🔧Mechanics/🖥️Computational_Mechanics/🚧Basics of Structural Reliability/第三章 可靠性灵敏度分析的矩方法#二、针对非正态变量的Rackwitz-Fiessler方法|针对非正态变量的Rackwitz-Fiessler方法]]进行解决, 然后再进行扩展

...
### (2) 核密度估计法拟合概率密度函数
为了优化原课本中的直方图算法, 我们使用更加优越的核密度估计算法, 即使用核密度函数拟合概率密度函数, 来估计取到对应点的概率。示例程序如下: 
...
按照权重抽样过程, 我们可以每一次生成一个随机数r, 然后将 w1, w1+w2, .... w1 + w2 + ...+wn (只需使用cumsum)排成向量形式, 然后检测获得的r落在哪个区间中确定重抽样时选样本点是哪一个。具体方法如下:
```matlab
...
### (3) 全局灵敏度计算
另外，生成 fx 分布的样本之后, f(X_i|F)的分布就可以直接估计了, 相应的 f(X_i) 可以直接使用均值mu(i), 方差sigma_d(i)的正态分布来代替, 只需将Pf改用重要抽样计算结果即可。

...
其中，第一步的具体方法是: 在每一步先重要抽样, 然后<b><mark style="background: transparent; color: blue">对重要抽样样本进行K-Means聚类分析</mark></b>(参考[[📘ClassNotes/⌨️Programming/👨‍🎓Deep Learning/Dive into Deep Learning(Pytorch)/Algorithms/1.KNN算法和KMeans算法|1.KNN算法和KMeans算法]]), 获取K个形心, 然后**将K个形心以及对应的真实功能函数值加入训练样本集T中**, 并**使用$T$更新代理模型**。 

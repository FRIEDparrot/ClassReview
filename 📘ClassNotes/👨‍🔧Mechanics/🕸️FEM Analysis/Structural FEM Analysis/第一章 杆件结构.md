...
title: 节点力和节点载荷的区别
collapse: open
梁单元的节点载荷$\left\{Q\right\}^e$, 梁单元的节点力$\left\{p\right\}^e$
其中，对于上题, 满足上述关系: 
$$\left\{ \begin{matrix}
Q_1 \\
Q_2 \\
...
![[第一章 杆件结构 2023-03-13 19.16.30|250]]
$$\left\{ Q_i\right\} = \left[\begin{matrix}
X_i &Y_i & M_i
\end{matrix}\right]^T$$
...
<mark style="background: transparent; color: yellow">单元节点力和单元节点位移的表达式</mark>
...
\delta_j'
\end{matrix}\right\}}$$
其中$[k']^e$为<mark style="background: transparent; color: yellow">单元刚度矩阵</mark>
...
并可以推导得出<mark style="background: transparent; color: yellow">空间下的刚度矩阵变换</mark>
$$[k]^e = [T^e]^{-1}[k']^e[T^e]$$

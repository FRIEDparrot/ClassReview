...
惯性式传感器: 是接收是通过<mark style="background: transparent; color: red">传感器的内部有质量，弹簧和阻尼器构成单自由度自由振动系统接收被测振动。</mark>被测机械量与中间机械量是**二阶微分方程相联系**， 因此称为二阶惯性系统。
...
<mark style="background: transparent; color: red">构成位移计的适用条件 :</mark> 位移计的固有频率远小于振动频率$\gamma = \frac{\omega}{\omega_{n}} = \frac{f}{f_{n}}>> 1$, 因而<mark style="background: transparent; color: orange">存在测量频率下限的问题</mark>， 放大系数为: 
...
title: 计算刚度
collapse: open
为了获取较大上限频率， 往往使用刚度较大的弹簧，**大约在$10^4$数量级左右**。
这个结论用于弹簧刚度的计算题目
`````
...
而对于加速度计的冲击响应，冲击过程的微分方程以及得到的响应为: 

...
需要说明的是，变换前后系统的稳定性不一定相同， 但是如果非奇异矩阵$L(t)$满足**当$t\geq  0$时，$L(t), \dot{L}(t), L^{-1}(t)$有界（变换条件）**，则变换之后的系统和变换之前的系统的稳定性相同

如果非奇异变换矩阵$L(t)$满足变换条件，则称该变换为<mark style="background: transparent; color: yellow">李雅普诺夫变换</mark>， 李雅普诺夫变换**不改变线性系统的稳定性**。

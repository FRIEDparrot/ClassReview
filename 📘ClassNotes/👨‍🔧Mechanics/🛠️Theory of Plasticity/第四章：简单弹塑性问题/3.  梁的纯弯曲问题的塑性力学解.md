...
$$\boxed{\frac{bh^2 \sigma_s}{4}\left[1 -\frac{4}{3}\left(\frac{z_s}{h} \right)^2 \right] = \frac{q}{2}(l^2 -x^2)}$$
此时，令$x = 0,z_s =\frac{h}{2}$，则有$\frac{bh^2 \sigma_s}{6} = \frac{q}{2}(l^2 - x^2)$，代入$x=0$,得到屈服应力和最大应变的表达式: 
$$\boxed{\sigma_s  = \frac{3q l^2}{bh^2} \qquad q_{\max} = \frac{\sigma_sbh^2}{3l^2} }$$
其中$q_{\max}$为弹性极限载荷

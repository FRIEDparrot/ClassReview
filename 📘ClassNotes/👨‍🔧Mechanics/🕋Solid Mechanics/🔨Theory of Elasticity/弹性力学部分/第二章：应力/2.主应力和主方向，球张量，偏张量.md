...
分别使用张量表示,得到
$$ \sigma_{ij} l_j = \sigma_n l_i \tag{1.1} $$
移项得到主应力的求解公式 
$$ (\sigma_{ij} - \delta_{ij} \sigma_n) l_{j} = 0 \tag{1.2} $$
显然，得到的解为
$$
\left| \begin{matrix} \sigma_x -\sigma_n & \tau_{xy} & \tau_{xz} \\
\tau_{xy} & \sigma_y-\sigma_n & \tau_{yz} \\
\tau_{xz} & \tau_{xy}  & \sigma_z - \sigma_n \end{matrix}\right|  = 0 \tag{1.3}
$$
为需要求解的方程

将方程整理为:  
$$ | \sigma_{ij}-\sigma_n\delta_{ij}| = 0 \tag{1.4} $$
...
将式(2-27)展开，有
$$ \boxed{\sigma_n^3 - I_1 \sigma_n^2 + I_2 \sigma_n - I_3 = 0   } \tag{2.1} $$
...
特点：
$$ l_1 = l_2 = l_3 = \frac{1}{\sqrt{3}} \tag{3.1} $$
使用主应力表示时
$$
\begin{matrix} \sigma_8 = \frac{1}{3}(\sigma_1 + \sigma_2 + \sigma_3) \\
\tau_8 = \frac{1}{3}\sqrt{(\sigma_1 - \sigma_2)^2 + (\sigma_2 -\sigma_3)^2 + (\sigma_1-\sigma_3)^2 }   \end{matrix} \tag{3.2}
$$

#### 四、 应力球张量与应力偏张量
取静水压力
$$ \sigma_m = \frac{\sigma_1 + \sigma_2 + \sigma_3}{3} \tag{4.1} $$
使得对应的应力矩阵对角线和为0，则有
$$ S_{ij} = \sigma_{ij} - \sigma_{m} \delta_{ij} \tag{4.2} $$

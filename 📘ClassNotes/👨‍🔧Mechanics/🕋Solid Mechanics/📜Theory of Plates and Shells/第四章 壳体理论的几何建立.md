...
## 一、空间曲线与曲面
### (1) 空间曲线的标架建立 
对于曲线的一般表示, 参考[[📘ClassNotes/📐Mathmatics/📈Advanced Mathematics/第八章 向量代数和空间解析几何#六、空间曲线及其方程|空间曲线及其方程]]部分
1. 显式表示法: 对于空间曲线, 我们可以使用类似参数方程
$$y = f(x)\qquad z = g(x) \qquad  a \leq x \leq  b$$
进行表示
2. 隐式表示法: 利用两个曲面的交线。对于空间点满足隐式方程组
$$F(x,y,z) = 0, G(x,y,z) = 0$$
3. 参数方程法:
$$x =x(t) \quad y = y(t) \quad z = z(t)\quad  \alpha\leq t\leq \beta$$
![[Excalidraw/壳体理论 2023-10-10 20.35.38|350]]
在空间曲线上<mark style="background: transparent; color: red">取如图的切向量和法向量</mark>(说明:**法向量取指向圆心为正**),经过[[📘ClassNotes/👨‍🔧Mechanics/🕋Solid Mechanics/📜Theory of Plates and Shells/补充内容/Frenet标架与切向量公式建立推导.pdf|Frenet标架与切向量公式建立推导.pdf]]得到如下公式: 
...
证明: $(\vec{r}, \vec{t}, \vec{n})$满足$<\vec{t}, \vec{t}> = <\vec{n},\vec{n}> = 1$ (尖括号表示点积)
`````
### (2) Frenet 标架
...
仍然通过[[📘ClassNotes/👨‍🔧Mechanics/🕋Solid Mechanics/📜Theory of Plates and Shells/补充内容/Frenet标架与切向量公式建立推导.pdf|Frenet标架与切向量公式建立推导.pdf]]得到<b><mark style="background: transparent; color: blue">Frenet标架公式</mark></b>: 
...
### (3) 曲面的表示与第一二次微分型
#### 1. 曲面的第一基本量和第一二次微分型
对于空间中的任何曲面，我们在高数中已经得到过曲面的方程表示：对于空间曲面, 有三种方程
1. <mark style="background: transparent; color: red">显式表达</mark>: $z = f(x,y)$ 
2. 一般式: $F(x,y,z) = 0$
3. 参数方程式: 
$$\begin{cases}
x = x(u,v) \\ y = y(u,v) \\ z = z(u,v)
\end{cases}\qquad  u,v \in D_{uv} \subset R$$
在空间曲面中我们用参数方程表示曲面，我们取参数方程中的$u,v$为$\alpha_1, \alpha_{2}$, 则上面第3式可以写成: 
$$\Large\boxed{\vec{r}= \vec{r}(\alpha_{1}, \alpha_{2})}$$

...
### (2) Frenet 标架
密切面定义: 取**另外一个标架**$b = <\tau, n>$, 并定义<mark style="background: transparent; color: red">密切面</mark>是单位主法线$\vec{n}$和$\vec{\tau}$所在的平面(<u>以副法向量为法向量的平面</u>, 即曲线所在的平面), 也可以定义为$P$无限接近于$P_0$时, $P$点与$P$点邻域内的点构成的平面当$P \rightarrow P_0$的极限位置。
...
而**Frenet 标架**可以取为
$$\boxed{\left\{\vec{r} , \vec{t}, \vec{n}, \vec{b} \right\}}$$
### (3) 曲面的表示与第一二次微分型
#### 1. 曲面的第一基本量和第一二次微分型 

...
\end{cases}$$
则积分得到下列公式 : 
$$\boxed{s_2 - s_1 = \int_{1}^{2} c_v \frac{dT}{T} + R \ln \frac{v_2}{v_1} \quad (真实比热容)= c_v \ln \frac{T_2}{T_1} + R \ln \frac{p_2}{p_1} \quad (定比热容)}$$
$$\boxed{s_2 - s_1 = \int_{1}^{2} c_p\frac{dT}{T} - R \ln \frac{p_2}{p_1} \quad  (真实比热容)  = c_p \ln \frac{T_2}{T_1} - R\ln \frac{v_2}{v_1}\quad (定比热容)}$$
通过[[Excalidraw/第三章 理想气体的性质与过程 2023-07-26 19.28.44|熵微分关系式的简单推导]]容易得到: 
$$\Large \boxed{ds = c_v \frac{dp}{p} + c_p \frac{dv}{v}}$$
以及对于理想气体的定比热容计算公式
...
代入绝热的过程方程， 上式可以写为:
$$w = \frac{RT_1}{\gamma-1} \left[ 1-\left(\frac{p_2}{p_1}\right)^{\frac{\gamma - 1}{\gamma}} \right]\tag{2}$$
需要说明的是，<mark style="background: transparent; color: red"> (2) 仅适用于定比热理想气体的可逆绝热过程</mark>

4. 技术功
由于q = 0, 参考[[📘ClassNotes/👨‍🔧Mechanics/🌊Thermal and Fluid dynamics/♨️Engineering thermodynamics/第二章 热力学第一定律#四、稳定流动能量方程|第二章 热力学第一定律]]有$q = \Delta h + W_t = 0$
$$w_t= - \Delta  h = h_1 - h_2$$
对于定比热容的理想气体，有
...
$$w_t = \frac{\gamma RT_1 }{\gamma-1}  \left[ 1 - \left( \frac{p_2}{p_1}\right)^{\frac{\gamma-1}{\gamma}}\right]\tag{2}$$
<mark style="background: transparent; color: red"> (2) 仅适用于定比热理想气体的可逆绝热过程</mark>

其中， 显然有关系:
$$\Large\boxed{w_t = k \cdot  w}$$

...
$$ u = q + (1- x)h + \int_{x}^{1} \left\{\int_{0}^{y}l(z) dz\right\}dy \tag{1.6} $$
...
$$ \boxed{\int_{0}^{1} (u_{,x})^2 dx < \infty } \tag{2.1} $$
`````ad-note
title: H1 function
collapse: open
...
$$ \delta = \left\{u | u \in H^1, u(1) = q \right\} \tag{2.2} $$
...
$$ \mathcal{V} = \left\{ w|w\in H^1, w(1) = 0 \right\} \tag{2.3} $$
...
$$ \int_{0}^{1} w_{,x} u_{,x} dx = \int_{0}^{1}u_{,x} dw =\left. u_{x} w\right|_{0}^1 -  \int_{0}^{1} u_{,xx} w(x) dx = \int_{0}^{1} wl dx + w(0) h \tag{2.4} $$
$$
...
$$ \int_{0}^{1} w_{,x} u_{,x} dx = \int_{0}^{1} wl dx + w(0)h \tag{2.6} $$
for all $w\in \mathcal{V}$, we integrate it by parts and also make use of $w(1) = 0$, then
$$ 0 = \int_{0}^{1} w(u_{,xx} + l ) dx + w(0)\left[ u_{,x}(0) +  h\right] \tag{2.7} $$
...
$$ \LARGE\boxed{a(w,u) = (w,l) + w(0)h} \tag{2.9} $$
`````ad-note
...
$$ (W_h): \quad  \boxed{a(w^h, u^h) = (w^h , l) + w^h(0)h} \tag{3.2.3} $$
...
$$ w^h = \sum^{n}_{A=1} c_A N_A \tag{4.1.1} $$
...
$$ N_{n+1}: \bar{\Omega} \rightarrow R , \qquad  N_{n+1} (1) = 1 \tag{4.1.2} $$
thus the $q^h$ is given by 
$$ q^h = qN_{n+1}\qquad  q^h(1) = q \tag{4.1.3} $$
Using this definitions, we can write $u^h$ in the linear combination form : 
$$ \boxed{u^h = \sum^{n}_{A=1} N_A d_A + q N_{n+1}} \tag{4.1.4} $$
...
$$ d = K^{-1} F \tag{4.1.6} $$
where 
$$ K = [K_{AB}] = a(N_A,N_B) \tag{4.1.7} $$
$$ F_A = (N_A,l) + N_A(0) h - a(N_A,N_{n+1})q \tag{4.1.8} $$
...
$$ K = [K_{11}] = K_{11}\quad F = \left\{ F_1\right\}\quad  d= \left\{ d_1\right\} = d_1 \tag{4.2.1} $$
also we let $N_1(x) = 1-x$ and $N_2(x) = x$, then : 
$$ K_{11} = a(N_1,N_1) = \int_{0}^{1} N_{1,x},N_{1,x} dx = 1 \tag{4.2.2} $$
so 
$$
\begin{matrix}
...
$$ u^{h}(x) = \left[\int_{0}^{1}(1-y)l(y)dy + h + q\right](1-x) + qx \tag{4.2.4} $$
at that time, we assume the solution under the different $l$ : 
at $l(y) = 0$, then 
$$ u^h(x) = \left[\int_{0}^{1}(1-y)l(y) dy + h +q\right](1-x) + qx \tag{4.2.5} $$
we write it into the form:
$$ \boxed{u(x) = q(1-x)h + \int_{x}^{1}\left\{ \int_{0}^{y} l(z) dz\right\}dy} \tag{4.2.6} $$
...
$$ u(x) = u^{h}(x) = \frac{\rho(1-x)^2}{2} + h(1-x) + q \tag{4.2.8} $$
where the particular solution is: 
$$ \frac{\rho (1- x)^2}{2} \tag{4.2.9} $$
note that on the node $u(0)$ and $u(1)$, the solution are exactly the same : 
$$ u(0) = u^h(0) = \frac{\rho}{2} + h + q \tag{4.2.10} $$
...
$$ \boxed{K_{AB} = \int_{0}^{1}N_{A,x}N_{B,x}dx = 0\qquad(B>A+1)} \tag{6.1} $$
...
$$ \boxed{(w, \delta_y)= \int_{0}^{1}w(x) \delta(x-y) dx = w(y)} \tag{7.1} $$
...
$$ \Large\boxed{a(w,g) = (w, \delta_y) = w(y)} \tag{7.5} $$
...
$$ u^h(\xi) = N_1 (\xi)d_1 + N_2(\xi)d_2 \tag{8.1.2} $$
...
$$ \xi(x) = c_1 + c_2 x, \quad \xi(x_A) = -1, \quad \xi(x_B) = 1 \tag{8.1.3} $$
we can easily solve the system and then : 
$$ \xi = \frac{x_A + x_B - 2x}{x_A - x_B} \quad \overset{B = A+1}{\rightarrow }\quad \xi = \frac{2x - x_A - x_{A+1}}{x_{A+1} - x_A} \tag{8.1.4} $$
or if we set $x_{A+1} - x_A = h$, then we have : 
$$ \boxed{\xi(x) = \frac{2x - x_A - x_{A+1}}{h_A} \quad \text{and} \quad x(\xi)= \frac{h_A \xi + x_A +x_{A+1}}{2}} \tag{8.1.5} $$
`````ad-quote
title: stipulate
collapse: open
...
$$ \boxed{N_a(\xi) = \frac{1}{2}(1 + \xi_a \xi) \qquad a = 1, 2} \tag{8.1.6} $$
where $\xi_1 = -1, \xi_2 = 1$ is the local coordinates 
$$ \boxed{x^e (\xi) = \sum^{2}_{a=1} N_a (\xi) x_a^e} \tag{8.1.7} $$
...
$$ K = [K_{AB}]\quad (n\times n), \qquad F = \{F_A\} \quad (n\times 1) \tag{8.2.1} $$
where 
$$ K_{AB} = a(N_A, N_B) =\int_{0}^{1} N_{A,x} N_{B,x} dx \tag{8.2.2} $$
$$ F_A = (N_A, l) + \delta_{A1}h - a(N_A, N_{n+1})q = \int_{0}^{1} N_{A} l dx + \delta_{A1} h -\int_{0}^{1} N_{A, x} N_{n+1,x} dx \space q \tag{8.2.3} $$
...
$$ K = \sum^{n_{el}}_{e=1} K^e\qquad K^e = [K_{AB}^e] \tag{8.2.4} $$
$$ F = \sum^{n_{el}}_{e=1} F^e\qquad  F^e = \left\{F_A^e\right\} \tag{8.2.5} $$
where 
$$ K_{AB}^e = a(N_A, N_B)^e = \int_{\Omega^e} N_{A,x} N_{B,x} dx \tag{8.2.6} $$
$$ F_A^e = (N_A, l)^e + \delta_{e1}\delta_{A1}h - a(N_A, N_{n+1})^e q = \int_{\Omega^e} N_A ldx + \delta_{e1}\delta_{A1}h - \int_{\Omega^e} N_{A,x} N_{n+1,x} dx\space q \tag{8.2.7} $$
...
$$ k^e = [k_{ab}^e]\quad(\text{for element}, 2\times 2)  \qquad f^e = \{f_a^e\}\quad (2\times 1) \tag{8.2.8} $$
$$ k_{ab}^e = a(N_a, N_b)^e = \int_{\Omega^e}N_{a,x} N_{b,x} dx \tag{8.2.9} $$
$$
...
$$ \Large\boxed{u^{(e)} = u_i^{(e)} \Phi_i} \tag{9.4} $$
`````ad-note
...
$$ \Large\boxed{\Phi_i = \frac{x_j^{(e)} - x}{x_j^{(e)} - x_i^{(e)}}} \tag{9.5} $$
...
where
$$ \xi = \frac{x - x_1^{(e)}}{\Delta x} \tag{9.7} $$
```
...
$$ \int_{0}^{h} \left( \frac{du}{dx}\frac{dw}{dx} - cw \right)dx = 0 \tag{10.2} $$
...
$$ \int_{0}^{h} \frac{du}{dx} \frac{d(\delta u )}{dx}  + c \delta u dx = \sum^{E}_{e=1} \int_{x_1^{(e)}}^{x_2^{(e)}}\frac{du}{dx} \frac{d(\delta u)}{dx} + c\delta u dx \tag{10.3} $$
Note that for single element, the ***Element FEM Equation*** is : 
$$ \int_{x_1^{(e)}}^{x_2^{(e)}}\frac{du}{dx} \frac{d(\delta u)}{dx} + c\delta u dx = 0 \tag{10.4} $$
...
$$ \Large \boxed{A_{ij} u_j^{(e)} = f_i^{(e)}} \tag{10.5} $$
where 
$$ A_{ij} = \int_{x_1^{(e)}}^{x_2^{(e)}}\frac{d\Phi_i^{(e)}}{dx}\frac{d\Phi_j^{(e)}}{dx} \tag{10.6} $$
$$ f_i = \int_{x_1^{(e)}}^{x_2^{(e)}}c\Phi_i dx \tag{10.7} $$
**2. Matrix Form of the element FEM equation**
$$
A_{ij}^{(e)} = \frac{1}{\Delta h}\left[\begin{matrix}
1 & -1 \\ -1 & 1
\end{matrix}\right] \tag{10.8}
$$
$$
f_i^{(e)} = -\Delta h \cdot c \left[\begin{matrix}
\frac{1}{2} \\ \frac{1}{2}
\end{matrix}\right] \tag{10.9}
$$
...
$$ \Large \boxed{A_{ij} u_j^{(e)} = f_i^{(e)}} \tag{11.1} $$
and the **Total FEM Equation** has the form of : 
$$ \Large\boxed{ A_{nm} u_m = f_n} \tag{11.2} $$
...
$$ \boxed{A_{nm} = \sum^{E}_{e=1} \left[\Delta_{ni}^{(e)}A_{ij}^{(e)}\Delta_{jm}^{(e)}\right]} \tag{11.3} $$
$$ \boxed{f_n = \sum^{E}_{e=1}\Delta_{ni}^{(e)} f_i^{(e)}} \tag{11.4} $$

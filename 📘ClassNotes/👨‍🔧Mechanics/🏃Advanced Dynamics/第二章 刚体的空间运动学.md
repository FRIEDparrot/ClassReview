...
`````
通过上述推导过程，可以得到速度和加速度的合成公式：
$$\boxed{\Large\vec{v} = \vec{\omega} \times \vec{r}}$$
$$\boxed{\Large\vec{a} = \frac{dv}{dt} = \frac{d\vec{w}}{dt} \times \vec{r}  + \vec{\omega}\frac{d\vec{r}}{dt} = \vec{\varepsilon} \times \vec{r} + \vec{\omega}\times (\vec{\omega}\times \vec{r})}$$
...
对于绝对导数和相对导数的关系，
我们设某一动坐标系$Oxyz$相对固定坐标系$Ox_0y_0z_0$做转动, 运动角速度为$\omega$, 设有一变矢量$a$, 
$$\begin{cases}
\vec{a} = a_1 e_1 +a_2e_2 + a_3 e_3 \\
\vec{a}' = a_1'e_1' + a_2'e_2' + a_3'e_3' 
\end{cases}$$
则有
$$\frac{d\vec{a}}{dt} = \frac{da_1}{dt}\vec{e}_1 + \frac{d\vec{a_2}}{dt} \vec{e}_2 + \frac{d\vec{a_3}}{dt} \vec{e}_3$$
而在$\vec{a}'$中，由于$e_1', e_2', e_3'$是随时间变化的量，因此对其求全导数，有：
$$\frac{d\vec{a}'}{dt} = \frac{da_1'}{dt}\vec{e}_1' + \frac{d\vec{a_2'}}{dt} \vec{e}_2' + \frac{d\vec{a_3}'}{dt} \vec{e}_3' + a_1' \frac{d\vec{e}_1'}{dt} + a_2' \frac{d\vec{e}_2'}{dt} + a_3'\frac{d\vec{e}_3'}{dt}$$
...
其中，由于在同一坐标系下，显然有$\frac{d\vec{a}}{dt} = \frac{d\vec{a}'}{dt}$，因此，我们令
$$\boxed{\frac{\tilde{d}a}{dt}= \frac{da_1'}{dt}\vec{e}_1' + \frac{d\vec{a_2'}}{dt} \vec{e}_2' + \frac{d\vec{a_3}'}{dt} \vec{e}_3'}$$
为矢量$a$的相对导数， 则有绝对导数和相对导数的关系：
$$\boxed{\frac{d\vec{a}}{dt}  = \frac{\tilde{d}\vec{a}}{dt} + \vec{\omega} \times  \vec{a}}$$

#### 5. 刚体的角加速度合成定理
利用角加速度和角速度的关系，以及角速度合成公式：
$$\omega_a  = \omega_e + \omega_r$$
求解绝对导数，直接得到：
$$\frac{d\omega_a}{dt} = \frac{d\omega_e}{dt} + \frac{d\omega_r}{dt}\quad  \text{where} \quad \frac{d\omega}{dt} = \frac{\tilde{d}\omega}{dt} + \vec{\omega}\times \vec{r}$$
即有公式： 
...
#### 7. 速度和加速度合成定理(II)
对于牵连运动为定点运动的情况：
利用速度合成定理，并考虑到$Oxyz$绕$O$运动
$$\vec{v}_a = \vec{v}_e + \vec{v}_r  = \vec{\omega} \times \vec{r} + v_r$$
对于<mark style="background: transparent; color: red">空间中的任一动点</mark>$M$, 而<mark style="background: transparent; color: yellow">有一动系相对于定系</mark>$Ox_0y_0z_0$绕定点$O$运动，动系上任一点相对于定系的角速度为$\omega$, 角加速度为$\varepsilon$

$$\Large\boxed{\vec{a}_a = \vec{\varepsilon}\times \vec{r} + \vec{\omega} \times \left( \vec{\omega} \times \vec{r}\right) + \frac{\tilde{d}^2r}{dt^2} + 2\vec{\omega} \times \frac{\tilde{d}r}{dt}}$$
其中上式使用矩阵可以表达为：
`````ad-todo
title: 推导
collapse: open
